...
### C3 review fix 2: applied report and final contribution fence

An existing pending, failed_retryable or expired executing ledger is not an applied original report. In unknown, replay requires both the original full identity/digest and `result_status=applied`; a pre-created ledger cannot obtain its first contribution there. Claim-time serialization alone is insufficient because its transaction commits before business contribution.

The existing non-completing access-failure transaction now checks the current workflow projection and absence of any applied failure for the same tenant/task **after** its instance-version CAS acquires the instance row lock. These reads use that transaction's client and remain within the same transaction as version/comment/audit/ledger contribution. An already applied failure, or a task no longer delegated after success, rejects and rolls back the entire attempted contribution. A report already legitimately claimed while fulfilling can still be the first applied report when another request merely failed its version check; existence of an unapplied failed ledger never proves a report was applied. Ordinary non-access actions are unchanged. Instance→ledger lock order and immutable payload/first verifiedAt remain unchanged.


## C4: Graph membership visibility and runtime compatibility

A successful membership write can precede its visibility on another Microsoft Entra replica. The verified grant owner waits only inside the original execution after the single successful POST; it never resends the write. It queries the identical group/member path, retries only HTTP 404 with exponential backoff, and requires the returned subject ID to match. A finite monotonic deadline includes in-flight reads; late responses cannot produce verifiedAt, cancellation propagates, and other errors remain unknown. Recovery after leaving that execution cannot manufacture a successful baseline or verification timestamp from a later GET.

KAF configuration: `GRAPH_MEMBERSHIP_VERIFY_TIMEOUT_SECONDS=10` (0 < value <= 60), `GRAPH_MEMBERSHIP_VERIFY_INITIAL_DELAY_SECONDS=0.5` (0 < value <= 5), and `GRAPH_MEMBERSHIP_VERIFY_MAX_DELAY_SECONDS=2` (0 < value <= 10). The initial delay is capped by the maximum. These bound the original read verification, not write retries or a new Procedure. The existing read transport policy remains beneath this deadline. Source: [Microsoft Entra eventual consistency guidance, 2026-03-24](https://devblogs.microsoft.com/identity/designing-for-eventual-consistency-for-microsoft-entra/).

A valid approvedAccess binding selects its registered Procedure intent independently of legacy intakeSnapshot.operationKind. Invalid bindings fail closed; ordinary tasks still require their existing operationKind. The directory/workspace binding, exact Procedure content and currently authorized access snapshot remain authoritative.

The deployed KAF history response and frontend must match: GET /sessions/{id}/history now returns the existing authorized history plus active_interactions. Current-card restoration uses the same projection owner as /sessions/open. ITSM delegated list pages omit an absent nextCursor instead of serializing an empty cursor.

Controlled acceptance cleanup must use this run's known write attribution. An initial nonmember read after a successful add does not remove the obligation to invoke the native remove tool. After a known successful DELETE, bounded reads confirm absence without repeating DELETE. A previously confirmed removal is not repeated; unrelated preexisting access is not removed. The C4 runtime failure and later restoration evidence remain distinct.
