...
独立 CI 修复由 [PR #29](https://github.com/Joes9527/itsm/pull/29) 交付，替代保留失败历史的 PR #28。它仅为可丢弃环境调用现有受控准备 CLI，使用真实备份/恢复、独立运行身份与标准执行绑定；普通 bootstrap 和 P/R 约束保持不变。实际组装先后暴露夹具的迁移后过宽授权、MinIO endpoint 格式和缺失附件桶，修正均限定在 CI 预配，不放松运行校验。最终编排 16 项与 SDK 3 项测试、构建、秘密扫描及独立静态复审通过；这些不能替代实际 GA 的 P/R 回执、健康检查和 API smoke。具体初始化规则维护在 [开发指南](../DEVELOPMENT_GUIDE.md#ga-gate-的可丢弃数据库准备)，不复制为另一套发布流程。

后续实际 GA（CI 修复 `4f114e32` 与功能 `40a2f8b3`）均确认 P=1、R=0、最终初始化、前后端健康检查、登录及既有 API 检查成功，清理成功。整体 smoke 仍因 Next 根路径合法 307 与旧直接 200 断言不符而失败；最终仅为前端 GET 增加最多 5 次、15 秒的跳转跟随，仍要求终点 200/304，不修改后端 API 断言。新增正常跳转、循环及 404 三项真实 HTTP 回归，与 16 项编排、3 项 SDK 测试分组记录；最后提交的完整 GA 结果继续以 PR 为准。

功能提交 `76d3a492` 的远端 20 项检查通过，包括完整后端测试/构建、Frontend、Lint 和安全检查，Pages 部署按预期跳过；唯一失败是上述 MinIO 组装配置，未执行 health/smoke。后续仅合入 CI 存储预配与交接说明，应用源码保持一致。最终合并状态和最新 GA 证据以 PR #29、PR #27 为准，不能将此历史快照当作新提交已通过的证明。

## 4. 建议继续顺序

1. 源码交付按 PR #29 → PR #27 的顺序核实最终检查并合并，状态以对应 PR 为准；完成这一检查点后，从下面的匹配版本部署和真实角色验收继续，不重复设计已完成的源码实现。
