...
- 共发现 17 个 PG 镜像／配置相关容器：6 个运行，11 个停止（其中两个为 init 辅助容器，不能算作已确认数据库实例）。运行实例中有 20 个非系统逻辑库；停止候选实例另有 2 个历史已知业务／验证库；其余 10 个停止容器内部库名未核实。
- 操作：仅 Docker 元数据及强制只读 PostgreSQL 目录查询。未启动、停止、备份、改名或删除任何数据库，未干预任务二。
