...
- 状态：draft（业务规则已逐节确认，书面规格待用户审阅；尚未实现）。
- 日期：2026-09-17。
...
**审查补充，待书面确认：分类纠正不得破坏服务申请的完整性。** 门禁启用后新建的 Requested Item，主动纠正必须提交当前有效的完整三级路径；拒绝完整→部分和完整→清空，允许完整→完整。历史在途单继续按 §5.2 兼容，不因新增质量要求追加强制阻断。其他允许未分类的专业类在门禁时点之前可使用部分分类，不能绕过各域原有修改权限。

分类纠正的状态范围必须由专业所有者检查，不因共用组件而开放通用终态编辑。已关闭、已完成或已取消记录不新增纠正入口，现有合法重开规则保持不变。Incident 的已解决但未关闭阶段需要可补齐 CTI，否则关闭门禁无法修复：实施计划必须核对其专业修改入口，必要时增加仅修正分类的受控动作，不开放其他字段、不执行重开、不重置 SLA；这是支持已确认关闭时点的必要能力，须纳入书面审阅。Problem/Change 同样在最终关闭前保证合法补齐入口，不能靠回退专业状态修正分类。
...
- Important：Requested Item 创建后纠正可退化为不完整分类。§5.3 已写入保持三级完整的明确建议，待用户书面确认，不再由实施者自行选择。
- 状态边界：已补“终态不新增开放入口”，并明确 Incident resolved 后、close 前可补齐分类的必要性；细化必须随书面规格确认。
- 停用路径：明确适用于所有停用前已合法分类的工单，不仅是截止时间前的历史单。
- Generic：实施计划必须覆盖 resolved 和直接关闭路径，避免无法修正的关闭门禁。
- 回退恢复：补充建议是暂停门禁不改变原截止时间，暂停期间新建记录仍归新单；重新启用前盘点缺配且提示处理，不追溯撤销期间已完成的业务、不重算 SLA。恢复后在途新单后续动作遵循原定门禁。该恢复语义待书面确认，执行计划须包含暂停期间建单、完成及恢复后的用例。
...
书面审阅重点：本设计将“未引用节点移动”扩展为检查整棵子树的引用；将历史完整但已停用的分类视为可保留，以免停用阻塞旧业务；要求规则引用有明确匹配范围。这些细化以及 §10 的审查补充随书面规格一并审核。书面批准后转入 writing-plans，才制定具体迁移、API和任务顺序；当前不执行实现。
