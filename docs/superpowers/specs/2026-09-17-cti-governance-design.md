...
决定：**通用工单**（`recordClass=generic`，`PUT /tickets/:id` 与子任务编辑）的分类只接受最深节点 ID；分类确实变化（设置或清空）时必须带 `classificationReason`，并把原因与前后完整路径写入既有 `work_item.edit` 操作回执。**不保留**按显示名称解析分类的字段或路径——两套口径会让同名节点静默落到错误分支，并使完整路径无法留痕。

- 边界失败关闭：编辑载荷以"拒绝未知字段"方式绑定（与 intake/change/service catalog 既有边界一致），因此已退役的 `category` 字段与拼错字段会**显式报错**，而不是被 JSON 反序列化静默忽略后"看起来成功"。
- 身份不可注入：`userId` 等非契约字段同样被拒绝；操作者仍来自鉴权上下文。
- 无过渡机制、无并行路线、无别名映射：字段与解析路径同批删除；不需要保留期或退役条件。
- 回滚：纯代码回退（`git revert`），不涉及 schema、数据迁移或历史记录改写。
- 兼容性范围：仅通用工单编辑；专业域纠正命令（Incident/Problem/Change/Requested Item）与其入口保持不变；创建路径（`handlers/intake` + `CTIInput`）本身即 ID 契约。

### D：已登记但未实施的剩余项

两项与"分类只认节点 ID、失败关闭"同源但**尚未实施**的工作已登记为 backlog：**BL-CTI-02** 创建入口分类名称槽位退役、**BL-CTI-03** 分类筛选按节点 ID 与查询参数白名单。二者均不涉及共享库写入、迁移或部署；实施前需先关闭其待决问题。详见 [backlog 设计草案](2026-09-12-backlog-design-drafts.md)。
