...
- 历史工单上的完整旧分类即使后来停用，仍可作为历史有效路径完成质量校验；主动改选必须选择当前有效路径，避免停用使在途工单突然无法关闭。
...
书面审阅重点：本设计将“未引用节点移动”扩展为检查整棵子树的引用；将历史完整但已停用的分类视为可保留，以免停用阻塞旧业务；要求规则引用有明确匹配范围。这些细化随书面规格一并审核。书面批准后转入 writing-plans，才制定具体迁移、API和任务顺序；当前不执行实现。
