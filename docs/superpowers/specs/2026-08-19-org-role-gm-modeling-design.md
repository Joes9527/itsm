...
> **后续更新（2026-08-20）**：这个方案基于当时的分析（`department.manager_id` 尚未接入真实数据）成立，但对着嘉里物流真实 eHR 数据验证时发现矩阵组织下一个部门节点常年并存多条业务线的平级总经理——`assigneeDeptId` 对所有申请人给出同一个 answer，无法区分业务线，结构性不适用于"总经理审批"这一层级。**"总经理审批"场景请改用 [[2026-08-20-personal-manager-chain-approval-design]] 里的 `assigneeGmChain`**（顺着申请人自己的汇报链解析，天然按业务线区分）。`assigneeDeptId` 机制本身没有被废弃，仍适用于不涉及矩阵歧义的固定部门审批场景，只是不再是"总经理"这一层级的首选。
