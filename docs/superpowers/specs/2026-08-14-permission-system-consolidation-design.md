...
- `pkg/seeder/seeder.go`：`rolePermissionMap` 中 `ticket:write` 拆分为 `ticket:create` + `ticket:update`（与路由细分权限对齐），并修正「工单分类」菜单 PermissionCode 由 `ticket:write` 改为 `ticket_category:read`
