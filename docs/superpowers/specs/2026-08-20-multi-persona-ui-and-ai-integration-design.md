...
## 7. 后续开发者指南与工程实践准则 (Developer Guidelines)

为了确保后续新增功能在交互体验与架构风格上保持高度一致，所有前端及全栈开发者在贡献代码时必须严格遵守以下准则：

### 7.1 页面扩展与归属准则
* **普通员工/提单人体验 (`/portal`)**：
  - 若开发面向终端员工的功能（服务目录项、报修表单、知识库文章自愈、个人工单进度），必须归入 `/portal` 路由体系。
  - 遵循 `PortalLayout`：保持极简宽屏无侧边栏设计，屏蔽 IT 内部状态流转机细节，严禁引入复杂的 ITIL 专业术语与表格堆砌。
* **技术支持/运维高频处理 (`/workspace`)**：
  - 若开发面向 L1/L2 客服、DBA、网络工程师的排障处理功能，必须归入 `/workspace` 路由。
  - 遵循三栏分屏（Split-View）高密度设计，右侧固定承载上下文画像与排障辅助工具，保持操作键盘友好（支持快捷键与常用模板回复）。
* **管理与运营监控 (`/manager` & `/executive`)**：
  - 服务台主管侧重实时进单、SLA 违规预警与团队负载调配（Wallboard）；
  - IT 高管侧重 MTTR 改善趋势、多维度成本/许可证消耗与战略 KPI 洞察。
* **系统底座配置 (`/admin`)**：
  - 仅放置面向管理员的 BPMN 设计器、组织架构权限、连接器市场及全局配置。

### 7.2 场景化 AI 能力内嵌准则
* **严禁孤立弹窗与独立 Chatbot**：
  - 任何新增的 AI 能力必须作为**场景化内嵌卡片 (Embedded Cards)** 放置在最相关的工作区内（如自愈推荐在搜索框/表单下，相似故障推荐在工作台右栏，摘要在沟通流顶部）。
* **AI 铁律一：显式置信度 (Confidence Badge)**：
  - 所有 AI 推荐（如分类预测、优先级判定、根因推荐）必须使用统一的 `AIConfidenceBadge` 组件展示置信度百分比。
* **AI 铁律二：决策辅助，人工显式采纳 (Human-in-the-Loop)**：
  - AI 只能提供建议与草稿（如“引用此方案为回复”、“采纳推荐分类”），严禁在无操作者确认的情况下静默修改业务主表数据。
* **AI 铁律三：非阻塞异步加载与确定性降级 (Non-blocking & Fallback)**：
  - AI 模块必须使用独立 Skeleton 异步加载，不得阻塞主页面的秒开渲染；当 LLM 超时或向量服务不可用时，优雅隐藏或降级为静态提示，不得抛出全屏异常。

### 7.3 多重角色身份与视图切换规范
* 用户登录后由系统自动计算其最高权限工作台并默认进入。
* 任何具有多重角色权限的用户（如 IT 主管自身也需提单或处理工单），系统 Header 必须呈现 `PersonaSwitcher` 组件，确保其能在自服务门户与控制台之间秒级无缝切换，无需反复登出切换账号。
