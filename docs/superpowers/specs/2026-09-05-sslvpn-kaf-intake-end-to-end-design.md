...
> 状态：accepted — 用户在本设计交付后确认继续，进入实施计划阶段；尚未开始业务代码实现。
>
> 日期：2026-09-05。
>
...
本设计作为本次 SSLVPN 交付的范围依据；旧设计中对 Service Request 创建和 KAF 对话入口的阶段性排除不再适用于本次交付。若实施改变 AGENTS.md 的架构约束，必须同步 CLAUDE.md；本设计当前遵循已有约束。
...
### 7.5 MSP 创建身份与事务快照（实施细化）

本节落实 Unified Work Item 设计 §16.3 的操作者租户与目标租户分离；不改变 KAF Web 当前用户申请或执行凭据边界。

- 签名的客户租户会话确定业务数据范围；原生操作者身份、有效 MSP 分配及规范角色映射由现有 tenant-session 授权服务读取。专业资源权限仍在所选客户租户检查。HTTP 解析的目标必须与认证会话及 SQL 租户范围一致，不能凭请求头改变业务授权。
- 每次创建尝试先打开目标租户 RepeatableRead 事务，再由受限 System 只读事务导入其 PostgreSQL 快照，读取用户、租户与 MSP 分配。目标权限、配置与业务写入使用原事务；不扩大 System 表权限，不以独立快照或缓存替代。导入/读取失败时关闭资源并拒绝，重试重新建立完整快照。授权以该次尝试的快照时点为准，不承诺后来提交的撤权取消已进行的事务。
- 跨客户创建必须显式选择目标客户的有效申请人，且满足现有专业 `create_on_behalf` 权限。Incident→Problem 增加可选正整数 `requesterId`；同租户缺省仍为当前操作者，跨客户缺省拒绝并要求选择申请人。不得静默继承源 Incident 申请人，不替换操作 actor，不改变源记录申请人。
- Intake 回执保存不可变 `actor_tenant_id` 作为原始身份出处；现有审计所有者记录 `actorTenantId`、`targetTenantId`、`actorUserId` 及回执/WorkItem 关联。WorkItem `opened_by_id` 保留原操作人，`requester_id` 为所选客户申请人。业务关联继续受目标租户约束。
- 新建和回执重放均重新检查当前分配与权限。已提交的租户策略自动化沿用原回执授权，不因后续角色或分配撤销自动取消；保留原有操作者缺失/停用时可见阻塞的规则。持久化启动与 Incident 事件必须支持同一身份出处，不允许只放行创建而使必要履约步骤失效。
- 迁移必须验证历史身份出处；缺失或冲突记录产生整改清单并阻止升级。采用新版本迁移而不修改既有 024。下一版本 026 用于此身份/事件约束，尚未实施的共享字段清理顺延 027+；最终仍需完整升级、Ent 重启与真实 Runtime/System 角色验收。
...
用户已确认继续。执行入口为 [实施总计划](../plans/2026-09-05-sslvpn-end-to-end-implementation.md)，其中关联三份子计划与集成门禁。规划核查另外发现 `worktree-unified-intake-p1-reconciliation` 在 `35d1958e` 已有尚未合入 main 的归并成果；执行时按总计划审查复用，不当作 main 已实现，也不重复从零开发。
