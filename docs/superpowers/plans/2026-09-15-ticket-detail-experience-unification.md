...
维护者后续授权：同步文档、合并 PR #35 并应用到本地 3010。合并前集成最新 main 并重跑受影响前端验证；本地发布遵循 development-environment.md 的 canonical stack 启动契约，实际版本和验收结果记录到私有 active-release.json。此授权不包含后端替换或数据迁移。
