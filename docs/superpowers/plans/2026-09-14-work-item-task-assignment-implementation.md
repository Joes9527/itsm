...
## Main integration amendment — 2026-09-15

Status: accepted technical integration approach; implementation and integrated validation in progress. Owner: integration Coding Agent, independent authority/concurrency reviewer. User requested pull and merge to main after source delivery. This amendment preserves the approved business behavior while adapting it to main `14292cb4`; it supersedes earlier task signatures/caller inventory that refer to removed professional Update APIs, generic professional assignment shortcuts, and a new nested SessionReader transaction. Historical source validation does not certify this integrated tree.

### Authoritative transaction and identity

- Keep main professional commands and `workitemmutation.Meta` authoritative, including trusted actor/tenant, operationId, expectedVersion, source/correlation, reason, CAB/assessment facts, callback settlement, lifecycle and execution scope.
- Preserve current authorization before replay. Replay of a successful operation returns its existing receipt; changed intent conflicts. Only new mutations proceed through current state/version and execution scope checks. Do not validate a changed present-day assignee before a legitimate replay.
- Within the caller-owned transaction, use callback-scoped `workitemassignment.WithLifecycleWriter`, backed by existing lifecycle actor/directory/MSP resolution. Bind exact transaction client, actor/native tenant and target tenant in private immutable values; invalidate the capability on callback exit. Reject substituted clients/identities and expired use. Public IDs and an active-user lookup are not authentication; professional command authorization remains mandatory.
- Do not fabricate SessionSnapshot or create a nested transaction. Propagate directory and close errors so the outer transaction rolls back; do not retain a closed directory client or turn failed reads into cached permission denial.

### One write and one version

- The existing shared assignment writer is the only owner mutation location. Keep operation receipt and assignment audit/event as distinct facts in the same transaction.
- For an actual owner change, run the writer first; apply remaining metadata using its returned version without incrementing again. Without owner change, the owning command performs its one normal version increment. Receipt and event reference the final version.
- Set mutation-attempt tracking before the first side effect, preserving main concurrent operation-receipt recovery. A replay cannot duplicate assignment events or notifications.
- Retain main professional endpoint rejection where a generic route cannot carry the full professional command contract; do not restore removed Update methods or a second assignment API.

### BPMN, migration and verification

- Combine main canonical WorkItem identity, candidate execution scope and caller transaction with bound task locks and verified actual execution actor. A `forClient`/transaction helper must preserve existing context restrictions.
- The feature assignment-source migration numbered 032 was not deployed. Main already owns another 032 and subsequent migrations. Use `047_bpmn_assignment_source` in the established active sequence before terminal controlled retirement (main latest additional migration is 046), preserving all main historical bytes/checksums and controlled-retirement ordering. Update verifier, tests and deployment references together; re-run preflight against the actual target before deployment.
- Keep main dependency/toolchain fixes. Install frontend using the existing CI command and lockfile; source checks from the older branch do not substitute for integration tests.

Required integration regression matrix:

| Boundary | Required evidence |
| --- | --- |
| Operation receipt | Same operation replays without duplicate owner writes/events; changed intent conflicts; current authorization still enforced |
| Version | Stale observed version rejects; actual assignment plus metadata increments once; same-owner plus metadata increments once |
| Professional rules | Change reason/CAB/stage and callback settlement survive; Incident assignment state rules and Problem progress remain authoritative |
| Scope | Candidate out-of-scope command rejects; tenant/MSP allocation revocation rejects; identity capability substitution/expiration rejects |
| BPMN | A→B changes unfinished execution authority; terminal responsible/actual actor frozen; completion/reassignment and task creation races covered in PostgreSQL |
| Migration | Historical main checksums unchanged; unique active sequence and retirement ordering; new column/default/immutability verifier tested |
| UI | Preserve main canonical identity tests and feature assignment-state tests; run lint/type/full tests/production build with main dependencies |

Independent review of the final integrated source is required. Integration does not authorize or prove shared migration, deployment, catalog publication or live browser acceptance; those retain the existing acceptance-plan gate.

### Integrated review requirements

The first independent integration review found five concrete omissions. The following corrections are implemented and covered by the final integration regressions; final independent sign-off and remote merge gates remain recorded in PR #27:

1. Typed Ticket edits containing an assignee change must share the existing assignment permission and row-scope check, including current authorization before replay. Identity validation alone is insufficient.
2. Bound task commands must use current grants and active roles from their transaction, as main ordinary tasks do. Process-wide cached RBAC must not authorize a new command after revocation; cover warmed caches, removed grants and inactive roles.
3. Task detail loading, authorization, effective owner, terminal evidence and action projection must run within one read snapshot. Passing an already-loaded stale task into a later projection does not satisfy consistency.
4. Mixed/status edits involving an authorized MSP owner must use the existing notification materialization and dispatch boundary with the same current recipient eligibility; preserve delivery keys and replay semantics, and reject revoked or unrelated foreign recipients. Materialization must read persisted notification preferences in the owning transaction even when no preference service was injected. Fault-injection tests must prove they reached the intended rollback point rather than passing on an earlier scope denial.
5. Incident's centralized command error mapper must return the existing retryable conflict response for serialization failure without exposing database detail.

Additional integration regression evidence must preserve Incident's positive-assignee contract even though the shared writer can represent generic unassignment. Source fixes, passing tests and independent rereview must be recorded separately; none of these checks constitutes runtime deployment acceptance.

Final local integration evidence: complete Go tests and build passed; isolated PostgreSQL passed all 34 top-level tests without skips; frontend passed 251 suites / 3,427 tests with 13 existing skips, lint/type/build passed; gofumpt, source/test mappings and engineering/API contracts passed. All 82 existing main migration assets remain byte-identical. See the [durable report](../../review/2026-09-15-work-item-task-assignment-report.md) for the independently tracked baseline GA initialization and Pages publishing failures. Do not bypass required remote checks or count this source evidence as shared runtime acceptance.
