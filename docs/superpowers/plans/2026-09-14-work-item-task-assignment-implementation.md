...
**Files:** Modify reachable owner-writing paths in `service/ticket_service.go`, `ticket_workflow_service.go`, `ticket_assignment_service.go`, `ticket_assignment_smart_service.go`, `ticket_lifecycle_service.go`, professional assignment services and `service/bpmn/ticket_handler.go`/professional handlers as identified by preflight. Modify owning repository interfaces only where necessary to pass an existing transaction without nesting. Update matching existing tests and add regression tests beside owning services.
