...
- [x] 最后：在上述 UI 修复版本复用三角色真实路径验收（2026-09-14 运行集成版；纯人工三类配置，范围见末节）。end user 从门户进入目录并持久化建单；实际 Helpdesk 账号打开可见工单、处理/评论/附件并重新读取；被配置的审批人在既有流程完成决策并核对历史；已分配任务无需重复领取。按工单类别验证合法处理动作，不将审批绕到普通状态更新。发现的问题先分类，再补直接阻塞 UI 操作的修复。

每个实现交付使用独立可审查变更和相邻行为测试。聚焦回归之后运行相称的类型、静态、构建及契约门禁，完成独立审查再推进下一项。初次复核仅更新文档；后续实现与验证另记于下文，不宣称未执行项通过。

## 4. 环境、验证与完成边界

用户已授权当前开发环境进行真实浏览器验收。初期使用独立前端端口、不替换共享 API；后续用户授权提交、推送并更新开发版本后验收，当前部署与证据见末节。仍使用标记测试记录，记录前后端实际版本，不迁移/重置数据库，不触发无关外部授权或消息。复用外部 SSLVPN 测试前核对其副作用和清理边界，不能把历史测试直接无差别重跑。普通 IT 管理账号、菜单和行权限需实际核验，管理员成功不能替代。
...
### 简单人工任务领取与完成（源码 implemented，待部署真实验收）

- 动手前依据 AGENTS.md 再核对任务授权、生命周期、输入及审计实现。分支 `codex/feat/ticket-process-task-actions`，基于 `89e39f57`；复用既有 claim/complete 命令，不改变工单处理人与流程参与人的规则，不新建意见存储。
- 现有 BPMN 任务列表增加 `uiActions` 只读投影，调用已有 authorizeTaskCommandActorWithClient 与 ValidateBPMNTaskLifecycle。只为简单 user_task（空或 fulfillment purpose）提供本面板动作；审批、自动化及未知类型保留专用入口，带 formKey 的任务不提供空参数完成。旧后端没有 uiActions 时页面继续只读，不推断权限或默认开放。
- 独立审查发现逐任务解析身份造成 N+1，已修正为本次列表独有的 engine/resolver 副本复用一次身份解析；生命周期先判断。原 engine 不持有读缓存，真正 mutation 在其事务里重新解析。新增回归验证读取投影后停用账号仍不能领取，新读取报错。复审无剩余阻断。
- UI 依据后端动作投影显示领取/完成，完成前确认；提交锁防重复，会话/租户/权限/工单切换关闭旧弹窗，HTTP 认证重试保留提交上下文断言；403 清空任务，失败保留可恢复提示。成功只提示操作已提交，重新读取任务与工单，不把异步领域回调视为已交付。没有添加顶层 comment 输入或修改专用审批意见路径。
- 业务配置限制仍保留：工单分配给 Helpdesk 不会自动改派 BPMN 任务；若既有流程把受理任务分给申请人，页面按真实任务权限展示，不能据此声称该配置问题已修复。专用表单任务与三角色完整路径仍需后续验证。
- 验证：先红后绿；前端 2 suites / 35 tests 通过，包括领取/完成、重复提交、切换租户及 mutation 撤权；类型检查、lint、生产构建通过，lint 仅既有 BPMNDesigner warning。后端完整 service/controller/dto 三包测试通过，定向授权/领取/生命周期/决策测试通过。扩大回归发现先前 generic 创建权限修复遗漏的两个旧夹具，独立提交 `5838006d` 修正为 ticket:create（其他专业域保留 write），未修改生产角色配置。
- Chromium 2 passed：当前生产构建登录既有工单 #10，任务 GET 与领取/完成 PUT 使用隔离响应，验证领取→已分配→确认完成→刷新；未向共享任务提交动作。第一条用例继续覆盖错误重试与390/1440布局。私有3016/3017结束后停止，共享API没有替换。日志 `/tmp/task-actions-*`，不提交测试运行产物。当前部署缺少动作投影时不会显示按钮，不能把本轮隔离浏览器结果当作共享环境已完成真实流程验收。


### 运行集成与真实三角色复验（2026-09-14，纯人工 UI 核心路径通过）

- 用户授权 commit/push 后更新开发环境并验证；WSL 故障恢复后重新构建、启动并执行真实请求。功能分支 `codex/feat/ticket-process-task-actions` 已推送 `00819577`；运行集成分支 `codex/chore/ui-workbench-runtime` 的源码为 `80349336de3dbe5f7be6c5cc57b3f1783a8ebb6b`，合并实际旧部署源码 `3eb260d3`，保留分类、RCA 与 KAF 引用修复。独立审查确认两边独有文件、四个交叠文件及迁移 SQL 字节均保留；未执行迁移。
- 集成验证：authorization、handlers/intake、handlers/problem、handlers/change、handlers/service_request、service、controller、router、config 九个 Go package 测试通过；destination、TicketProcessTasks、TicketDetail 三组前端测试共 40 项通过。生产构建通过，部署构建显式设置 `ITSM_BACKEND_URL=http://127.0.0.1:8080` 和空 `NEXT_PUBLIC_API_URL`；build ID `wlcY0limNxk71JkwpGrDC`。API 二进制 SHA256 `62b3c53b1028d3ec67ba884037c767052b53230803cd1d3edcc9e35bca7f5334`，8080 readyz、3001 login、3001 csrf-token 最终均 HTTP 200。
- 回滚保留旧二进制和私有启动配置，备份目录 `~/.local/state/itsm-kaf-baseline-20260908/backup/ui-workbench-20260914T090159Z`。自动迁移、seed 保持关闭；本轮恢复 ITSM API/前端，没有部署 KAF 或调整后台 worker 配置。

| 真实场景 | 证据与结果 |
| --- | --- |
| 员工直接求助 | `end_user` 从门户填写标题/描述，创建 HTTP 201；详情刷新后标题、描述可见。正式复跑 WorkItem #17/#19/#21 通过；先前 #13/#14 的标题断言漏掉编号，属于测试选择器问题，已纠正并重跑。 |
| 员工目录申请 | 新建专用临时目录，沿真实申请页面提交并刷新详情，HTTP 201。三类路径 WorkItem #18/#20/#22；没有直接造 ProcessTask 或拦截写请求。 |
| 已分配 Helpdesk 协作 | `l1_support` 在个人工单队列打开已分配服务请求，提交评论、上传文本附件，刷新后重新看到；1440/390 宽度无 body 横向溢出。分配由管理员 API 设置测试前置条件，实际处理操作由 Helpdesk 浏览器完成。 |
| 先受理后审批 | 专用纯人工流程 #18：Helpdesk 在详情确认完成受理任务，`PUT /tasks/:id/complete` HTTP 200；`dept_manager` 在审批中心批准，`POST /tasks/:id/decisions` HTTP 200。刷新后待办消失，审批意见重新读取存在，实例 #18 为 completed/end。 |
| 直接审批 | 专用纯人工流程 #20 没有受理节点，管理层直接批准；意见持久化，实例 #20 为 completed/end。 |
| 无需审批 | 专用纯人工流程 #22 由 Helpdesk 完成人工任务后结束，实例 #22 为 completed/end；不伪造审批步骤。 |

本次真实样本未出现需要手动领取的待领取任务，因此仅将真实完成/审批记为通过，不把隔离 UI 用例的领取结果升级为真实领取证据。手动领取按钮仍以此前组件/后端/隔离浏览器测试为证。此验收证明三类配置的界面与既有任务执行契约，不证明全部专业生命周期、表单任务或外部交付完成。

既有目录 #14 的流程仍在 Activity_Accept 等待其配置参与人；本轮没有修改其分配规则。专用测试定义只含人工任务与结束节点，不包含 KAF、SSLVPN 授权或其它外部副作用。原有目录的实际参与人配置和专用表单仍需分别验收；KB/智能建单与团队负载保持 backlog。
