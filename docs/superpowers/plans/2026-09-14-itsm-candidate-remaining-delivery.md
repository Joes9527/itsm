...
**Tech Stack:** Go/Gin/Ent、Next.js、PostgreSQL、Redis、MinIO；当前 Agent 在 Windows/WSL 独立 worktree，同时负责代码、环境、迁移、部署与验收；不存在另一个待交接的 B。
...
- 生产发布就绪纳入交付目标；实际生产切换需要明确目标环境、数据范围及发布窗口，由当前 Agent 执行环境操作。目前没有这些生产目标事实，不能把WSL候选验收报告成生产上线。不合并 main；源操作仍遵守明确数据范围及一致性窗口。
...
- 不推送、不合并 main、不清理已有 worktree，不执行 R(038)、历史回填或企业实发。当前 Agent 接管原 B 的环境职责；仅在对应准入及窗口满足后执行环境操作。
...
| R2 / 完成（当前核心范围） | A：业务与异步入口的最小安全收口 | 只修R1证明影响本轮核心功能、成员/租户/当前权限、事务或历史保全的可达入口。BPMN已验证修复复用；飞书同步转D1，仅保留部署未启用边界核实。核心入口正向可用，拒绝路径无业务副作用；不再以飞书完整协议阻塞上线准备。 | S3、S4、S5 核心业务/裸实例/必需消费者授权 |
...
| R8 / 完成G3（7.16） | 当前 Agent 执行并复核证据：稳定运行及恢复交接 | 固定构建受控重启，关键旅程和消费者恢复；连续至少60分钟低频只读观察，保全人工新增数据，给出固定 URL/启停/资源/恢复说明及限制。 | T5/G3 |
| R9 / 待维护者实际验收（7.16） | 维护者验收、A 汇总：最终关闭 | 维护者完成代表性页面/业务旅程；G1/G2/G3 全部有证据，无未解释的阻断问题；分别报告代码、私有验证、候选运行、main、企业外发、生产部署状态。 | 原设计第7–9节 |
...
| D6 | main合并、新平台功能、长期运维/非必要性能优化 | 继续延后；生产发布就绪已按最新指示纳入目标。真实生产切换待明确目标/数据/窗口，由当前 Agent 执行，不以本表推定main合并或共享库操作授权。 |

除第1节飞书范围调整与第7.11节维护者接受的鉴权未完成项外，其他延后不改变核心安全设计；不能仅靠改名为backlog取消核心功能或安全要求。
...
| M4 稳定交付（G3完成，维护者验收待定） | R8–R9 | G3、维护者验收、稳定访问/恢复入口 | 关闭完整目标 |

执行约束：

1. 当前 Agent 同时只实现一个 R 项，并承担环境准备和执行；环境实际执行沿原授权/窗口。不得让多个 agent 同时编辑相同文件。
...
独立只读reviewer `m1_core_review` 核对 `3098f9718b98fbcd2fa4fc422a8496c14a141d7f..a830814b4b29948dd5c16aa4a5aee6f721848b3a` 及当前范围，结论：M1可关闭，无Critical/Important核心阻断，可进入M2；不追加新的M1开发或测试波次。
...
### 7.11 维护者确认：统一交付责任与已接受鉴权限制（2026-09-14）

维护者原话：“没有B，B就是你，你应该完成工作，‘新鉴权存储还需接入登录、刷新、注销，再验证重启与恢复’ 我可以接受，但需要在文档中标记出来”。本节是当前执行依据，取代历史检查点中等待其他 B、下一步必须先完成 A3/A4 才可推进代码交付的安排。

- **责任：**当前 Agent 同时承担原 A/B 的代码集成、只读准入、备份恢复、迁移、部署、业务验收及运行交付。R6 不再等待其他 Agent；仍须形成真实 T3 EnvironmentRevision，不能用责任合并替代证据。
- **已实现：**A1 严格 JWT 校验与验签租户绑定；A2 PostgreSQL TokenStateStore、046 空表迁移及私有角色/并发/故障测试。代码为 `d4bedf7b7da4d3a3560f93ef834c6d3f8f585c23`，具体证据见7.9–7.10。
- **明确未完成、维护者接受暂留：**新 PostgreSQL 鉴权状态存储尚未接入应用启动、登录/发证、刷新和注销；真实 API/数据库重启、鉴权状态丢失、备份恢复后的旧 token 拒绝与新登录验证尚未完成。A3/A4 保持未完成，不标为通过；本轮不再以这两项自动阻塞 R5/R6 的准备和后续候选交付。
- **当前实际行为与业务风险：**运行仍使用原 Redis/内存鉴权路径。实现了 PG 表和存储接口不等于运行中已使用。首次 Redis 连接失败可能保留内存撤销存储；进程重启或 Redis 状态丢失后，未过期 token 的注销/刷新防重放状态可能无法完整保留。不能承诺“注销永久生效”“跨实例/恢复后会话安全已验证”。
- **范围边界：**本次接受仅针对上述鉴权接线与恢复验证缺口，不豁免跨租户隔离、历史数据保全、候选资源/企业出站隔离、R(038)禁执行或源一致性窗口。候选使用独立 JWT 秘密；实际配置和验证结果须在 T3/T4 记录，不能仅凭本段视为已实施。
- **交付披露与后续：**R5 固定版本交接、运行说明和最终验收必须携带本限制。后续仍沿原 R4/A3–A4 接入并验证，不另建第二份剩余清单。R8 普通应用/消费者重启与持续观察仍执行；不把它们冒称为新 PG 鉴权恢复测试。

当前 M1 完成；M2–M4 未完成。历史 T2 revision-2 的源映射与证据需重新核实，旧拟议窗口已过期且从未批准/执行，不能沿用。没有因本次文档修订自动启动源停写、恢复、迁移或应用。
### 7.12 R5/M2固定版本与R6源复核（2026-09-14 11:43 CST）

本批约12分钟，仅推进既有R5及R6只读准备，未扩大功能范围。

R5完成：CandidateSHA `3142247e7ef4f29639d373d8bf53f797e35c0913`，代码worktree `candidate-release-windows`；T1交接文档提交 `e3ae6dda83264474b2890dc9d4ba05a518c04562`。离线包 `~/.local/state/itsm-candidate-delivery/handoff/candidate-r5-3142247e.bundle` verify成功，前置main `a25e108d`。原main及既有未跟踪docs/poc保留。

- 独立reviewer m1_core_review完成一次M2只读审阅：A1/A2和运行说明无本轮可达核心阻塞；确认新PG鉴权没有接生产入口，7.11披露准确。不将已接受A3/A4缺口重新立项。
- WSL依锁npm ci、类型/主题检查、前端production standalone构建、lint均退出0；lint保留1个既有warning。7个受影响前端套件159项通过。Linux API、Worker、带migrate标签的迁移工具构建通过；首次遗漏该标签失败已修正，没有改生产代码。
- 证据：agent-a-windows/r5-*.log；t3/build-3142247e/build-manifest.json含构建与日志摘要及退出码。Jest生成的已跟踪junit产物已恢复，工作区无源码改动。复用前文后端race证据；不冒称本批重跑全部后端或前端测试。
- 纯编译导出的当前迁移catalog与真实源24条ledger逐项checksum相等，旧SQL均未变。完整目标037/普通迁移实际准入及恢复仍属于R6，未执行任何迁移。
...
### 7.15 R8观察中断与恢复（2026-09-14 14:13 CST）

约14:07 WSL意外重启，8个当前候选容器退出255，原源API/两个Worker也中断。维护者明确回复“没有主动调整，请继续恢复”。根因尚未确定；Windows System对应短窗口未取得可解释事件，不能归因于用户操作、应用缺陷或宣称已修复宿主问题。

第一次观察仅34次通过采样、最后单调时长2037.58秒，不满足3600秒。t5/observation-samples.jsonl原样保留；observation-interruption.json状态interrupted_not_passed，没有生成或伪造通过报告。第二次独立目录t5/observation-2从零计时，不能拼接两段。

当前Agent按维护者授权恢复：用备份时保存的原argv/env/cwd启动原源API及两个Worker；仅启动当前SHA0788的原8个容器和原数据卷，不启动保留的旧版/失败容器，没有重新恢复dump、迁移或清理数据。host-recovery.json记录源/候选health200、相同容器ID及新启动时间。代码/配置未变，EnvironmentRevision仍为f058c822…24424，宿主恢复事件另留证而不覆盖原freeze。

恢复验证：原152表业务行无变化；唯一编号计数器8→29与21个新增工单一致（original-row-preservation-141017.json及reconciliation）。21个已记录新工单身份/编号/类别和2个附件记录保留。Redis222键DUMP及绝对过期时间通过，1键自然过期；网络探测自身PG/Redis/MinIO可达、源端口及外网不可达。recovery-after-host.log真实新RequestedItem审批/履约1/1通过，又新增一个合法候选记录，结束对账须计入。第二次完整观察14:13启动，R8/M4仍未完成；普通恢复不替代7.11鉴权A3/A4。
14:30批次报告：13:32–14:30约58分钟，范围仅R8运行观察及其实际宿主中断恢复。第二次观察约15.5分钟、采样正常；未达到3600秒，R8/M4不判完成。下一批继续同一观察、结束保全和一次独立复核，不增加功能或宿主配置改造。首次中断原因未知仍作为运行限制披露。
### 7.16 R8/G3完成与R9交付（2026-09-14 15:23 CST）

本批14:30–15:23约53分钟，范围只有剩余R8观察/结束对账和独立复核，没有增加功能、改宿主设置或重做未变化测试。CandidateSHA仍为`0788a9bb196ab37a8389b3f366bed9877b2f72c3`，EnvironmentRevision仍为`f058c8227e7f350387ecfa9f5296bc0a1fd852c1c392d1071619de27f1c24424`。

R8/G3在声明范围内完成：第二次独立观察14:13:08–15:18:14 CST，单调时长3601.71秒、60样本、0失败，API/Web/源health全部200；8容器身份/启动时间/restartcount在完整窗口不变，无新增API/Worker error日志。原首次2037.58秒中断单独保留，未拼接时长。实际日志证明12次SLA扫描启动、4次升级处理完成；不把扫描启动日志说成完成日志。构建、运行配置、镜像与网络指纹复核一致。

最终保全：original-row-preservation-151840.json仅原编号计数器8→30及时间变化，22新工单/最大编号30精确解释；原152表业务行无变化。22个已记录新增工单身份/编号/专业类别、2条附件记录均保留；两份附件原始字节实际API读回通过1/1。Redis222键DUMP/绝对期限及历史Stream保全，1键自然过期、无差异。原非零报告保留，按时点保存reconciliation；冻结时旧对账文件的精确hash副本可由t5/reconciliation-artifact-index.json定位。

独立m1_core_review直接核验第二轮样本、SQL/Redis/附件和冻结摘要，确认G3可在已声明范围关闭、无新增阻断。交付证据见环境文档分支docs/review/2026-09-14-candidate-t5-evidence.md；URL、启停与恢复限制见docs/deployment/itsm-candidate-runtime.md。

R9仍待维护者实际代表性页面/业务验收，M4不能由Agent代签。建议入口：工单详情及附件、变更审批时间线、服务请求审批/履约结果。新PG鉴权尚未接入启动/登录/刷新/注销及恢复测试；运行继续旧Redis/内存路径，7.11接受限制不变。Incident自动应急未交付，当前人工生命周期是已告知的可逆上线优先假设，其维护者偏好尚未明确确认；KAF仅隔离传输接收、企业投递未做、飞书不在本轮。WSL重启根因仍未知，本次成功恢复与后一观察不表示宿主问题已修复。

代码与私有验证、候选运行可交接；main未合并或推送，原工作区及docs/poc未提交文件保留。当前仅本机候选，不是生产切换；没有实际生产目标事实，生产上线未执行。R(038)、历史清理和真实企业写入均未执行。
最终提交前发现其他工作已推进main到a0c1371ee6ed056bb29898a70780faba20157092（14:40、15:00两条migration文档/脚本提交），本任务未生成、覆盖或回退它们。此前“main未变”仅对应当时检查点，不代表外部任务不会修改共享仓库。同时本任务refs目录及4个专用reflog文件变为root所有，造成文档commit拒绝；只恢复该目录和这4个本任务日志的administrator所有权，没有递归更改权限或操作其他分支引用。代码CandidateSHA与验证范围保持不变。