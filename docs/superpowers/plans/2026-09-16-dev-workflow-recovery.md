...
**Status:** accepted（总体方向）；详细设计已补齐并独立复核，按步骤实现。全部设计裁定见[完整实施合同](../specs/2026-09-16-dev-schema-execution-contract.md)§2；执行Agent负责实现/测试，不能另选流程或把设计留到共享操作时决定。
...
- [ ] 只读导出并核对定义65及受影响绑定，落实完整合同§2的定义级/任务级枚举、同一事务领域门禁和只读UI投影；修改现有解析/发布校验、`service/ticket_service.go`及其他generic状态入口、`TicketDetail.tsx`对应现有命令。分派/解决/关闭/升级必须保留领域receipt与授权，不能仅改XML。
- [ ] 按完整合同§2.1固定节点表生成tenant1 `ticket_general_flow_v2` v1.0.0 XML；源65及SHA必须一致，只切825/827，tenant2不自动复制。原拒绝流向处理是已确认缺陷，明确改为拒绝结束；保留升级条件，加入解决/关闭确认。生成XML是实现任务，不再让执行Agent选择业务图。
...
## A4：冻结无效回调转现有blocked，保留失败证据

**Files:** 既有callback执行器（由`executeClaimedCallback`定位）、`service/bpmn_callback_outbox.go`及outcome测试、流程/任务读DTO及现有详情UI。

- [ ] 按完整合同§2.3，在领取后、调用handler前校验冻结参数；已知handler/action的确定输入缺陷返回现有handler_contract blocked效果，不补payload、不运行handler、不推进流程。
- [ ] 通过现有outbox outcome事务保存blocked与审计；保留原payload/key/历史attempt和task33 completed。未知外部结果不归类为缺参数，正常瞬态故障仍沿用既有重试。
- [ ] callback2下一次受控处理进入blocked后不再被scan/claim，流程27保留阻塞状态并在UI显示；GET不产生变更，现有终止保护不放宽。
- [ ] 覆盖租约/CAS、并发执行、审计失败回滚、原载荷不变、重复处理不重复审计、blocked不被重试，以及有效callback仍可执行。
- [ ] 最终将29/27/33/2列为具名保留失败样本；这是已确定处置，不再设计新恢复接口，也不宣称旧实例成功结束。无关旧数据库清理不因此被整体阻塞。
