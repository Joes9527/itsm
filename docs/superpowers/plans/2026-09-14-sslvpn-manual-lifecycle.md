...
- [x] 核对历史出差值班故事、账号、业务字段与当前 KAF 履约边界。
- [x] 主手册增加 S0 数据卡、S1 表单五幕、S2 KAF 替代入口、S3 拒绝/字段/撤回分支。
- [x] 邮件手册增加 S4 员工报障演练→自动确认→信息补充→工程师协作→收尾。
- [x] Agent 执行顺序与记录模板改为按场景/幕交接，并映射原断言编号。
- [x] 检查文档链接、字段一致性及差异；本轮不执行实际业务测试。
