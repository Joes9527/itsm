...
> **当前设计入口：** [原Dev恢复与双数据库收敛设计](../specs/2026-09-16-dev-restoration-two-database-design.md)。维护者已确认方案A及数据/清理原则，书面设计待审阅；批准后沿用本清单制定实施步骤，以下历史建议不单独作为执行命令。

> **2026-09-16 后续裁定：** 已完成[Dev与主干分离原因双重核验](../../review/2026-09-16-dev-schema-divergence-report.md)，维护者要求先讨论解决设计。下述升级／克隆顺序保持proposed，不能按执行指令推进；总体目标不变。

> 最新工作顺序以[2026-09-16 目标对齐与两库收敛方案](#dev-clone-alignment)为准，取代此前后续优先级；既有 U/C/V 编号继续使用。下方运行/UI 记录保留为当时证据；本次文档更新不表示已切库、升级结构或重新验证运行状态。
