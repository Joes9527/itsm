...
> 最新工作顺序以[开发恢复与迁移验证约定更新](#development-restoration-update)为准，取代第 7 节及历史收口中的后续优先级。下方运行/UI 记录保留为当时证据；本次文档更新不表示已切库、升级结构或重新验证运行状态。
<a id="schema-target-status"></a>

## 当前结构目标与实际状态

**统一目标：[047_bpmn_assignment_source](../../development-environment.md#selected-schema-target-047)。** 以下为2026-09-15已记录的最后实测状态；本次文档修改不执行升级或切库。后续操作前刷新实际目标和迁移证据。

| 对象 | 最后核验结构 | 目标／状态 |
| --- | --- | --- |
| 实际Dev：`itsm_config_baseline_20260908` | 031 | **目标047，尚未升级**；禁止恢复旧后端来适配031 |
| 当前8080目标：`itsm_ga_ready` | 046 | **目标047，尚未同步**；这是隔离验证目标，不称正式GA |
