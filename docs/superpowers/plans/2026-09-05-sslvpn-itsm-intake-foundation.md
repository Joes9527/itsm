...
**Status:** accepted — 已按用户要求恢复；审查接手期间修改并继续 A3b/A4 后端入口退役与共享 schema 归并。[开发交接报告](../../review/2026-09-05-sslvpn-development-handoff-report.md)保留暂停时基线。
...
A3b/A4 后端按真实事务边界连续实施并分别复核：先完成可靠 BPMN 启动、冻结流程变量及 Worker 重放能力；再完成 Incident 自动化动作与领域副作用的事务恢复；最后成对切换全部后端入口和旧字段读写。现有 Incident 升级、时间线与告警分别提交，部分动作仅回显，因此不能用一层 Outbox 接线替代可靠执行。未实现动作必须显式失败；通知目标使用现有受治理配置，移除固定业务收件人。以上是原可靠副作用要求的实施拆分，原 A3/A4 与发布门禁保持开放。

可靠流程启动切片执行记录：`00034817` / `c1586c7e` / `56bc3df9` 已通过独立审查及两轮修复复核，覆盖精确流程定义与内容冻结、并发启动去重、Worker 崩溃恢复、数值条件及自动指派。迁移 `023_add_process_start_request_digest` 已在隔离库验证，后续迁移从实际注册表继续分配。当前继续 Incident 必要动作事务化；生产入口接线和原 A3/A4 门禁仍开放。

Incident 动作恢复切片执行记录：`e0055b83` 已通过独立审查，无阻断项。动作事务、冻结决策、回执、实际 Worker 重放和迁移 `024_incident_rule_action_receipts` 的重启/非绕过 RLS 验证通过。四项非阻断审查意见已在后续 RLS 前置切片中修复并通过复审；全入口与共享字段读写切换仍待完成。SQL 策略验证不替代实际应用/Worker 角色与租户上下文接线验收。

A3a 执行记录：`62fec316` / `ffc4d17d` 已通过独立审查及修复复核。九个相关包和真实 PostgreSQL 并发、配置交错测试通过；创建采用一致配置快照，专业默认值准备完成后再选流程，整次事务重试重新验证当前权限。原 A3 仍开放：A3b 与 A4 后端正在完成旧创建路径、共享字段和实际流程启动的切换。
...
运行隔离执行记录：`1808f2d9` / `367f9af9` 已通过独立审查及修复复审。实际 Ent 事务/连接池强制隔离、受限系统连接、API/Worker 构造、完整 `009 → 024` 策略下的认证与消费、Ent 会话变量清理均有回归证据。15 个受影响包通过，11 项受影响 PostgreSQL 用例通过；额外 `-count=2` 因既有测试驱动重复注册失败，单次运行和真实数据库覆盖独立通过。RLS 默认仍为 off，未部署；完整进程启动、后续迁移与全入口/前端/SSLVPN 验收仍开放。现在继续全入口与共享字段切换。
