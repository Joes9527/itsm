...
- Style roots at the planning baseline were `src/styles/theme-variables.css`, `src/app/globals.css`, `src/components/ui/design-system.css` and Tailwind declarations. That baseline contained conflicting 48px/260px layout values versus the approved 60px/224px values, plus literal-color Ant Design popup overrides; the implementation replaced those conflicts through the shared generated theme foundation.
...
Historical Task 6 static treatment (superseded by the final topology/BPMN evidence below): both ReactFlow hosts/nodes/labels/controls/minimap use semantic theme colors; topology and modeler data/layout are preserved. BPMN retains its white drawing sheet and dark inherited canvas text; custom and library chrome is theme-aware. This is bounded visualization treatment, not a route exclusion. Appearance and representative renderer interactions were subsequently covered by controller browser evidence. The six route files without local styling changes inherit their migrated active components or shared Antd foundation (including cloud-services).
...
| --- | --- | --- |
| Route error | `src/app/error.tsx` | STATIC MIGRATED / DARK OBSERVED during an earlier asset failure; light not formally verified |
| Root failure without layout/providers | `src/app/global-error.tsx` | STATIC MIGRATED / UNIT VERIFIED (8 tests); not browser-induced |
| Not found | `src/app/not-found.tsx` | STATIC MIGRATED / NOT FORMALLY SCREENSHOT VERIFIED |
| Main loading convention | `src/app/(main)/loading.tsx` | STATIC MIGRATED / representative catalog loading observed; not every loading state exercised |
| Root common error boundary with diagnostics | `src/components/common/ErrorBoundary.tsx` | STATIC MIGRATED / not every diagnostic state browser-induced |
| Auth-check/loading helpers and main footer | `src/components/ui/LoadingSpinner.tsx`, `src/app/(main)/layout.tsx` | STATIC MIGRATED / representative route loading only |

Task 7 validation: existing baseline 6 suites / 52 tests passed; final scoped regression 9 suites / 81 tests passed, including 8 new standalone-root-error tests and navigation-theme coverage. Type-check, generated theme consistency, scoped ESLint and whitespace checks pass. Details in `.superpowers/sdd/2026-09-09-visual-theme-unification/task-7-report.md`. The route-appearance browser matrix was subsequently completed. Live-backend validation remains outside this work.
...
- Evidence used an isolated exact-contract, authenticated, read-only fixture. It does not validate live backend integration, business writes, exports, SSO, every form action, or every deep component interaction. Raw untranslated labels and honest empty fixture states remain existing product/content limitations. The authored focused Playwright suite was not executed; system preference and storage-failure behavior is supported by unit coverage, not claimed as browser evidence. Of the supplemental conventions below, the route error was observed in dark during an earlier asset failure, but global error has eight unit tests and was not browser-induced, not-found was not formally screenshot-verified, and catalog loading was observed without exercising every loading or diagnostic state. These supplemental states are not included in the 144-route appearance claim. Local detailed evidence remains in `.superpowers/sdd/2026-09-09-visual-theme-unification/route-browser-matrix.json`, `browser-evidence.md`, and `task-8-report.md`; screenshots exist only in the controller CUA conversation and were not committed.
