...
后续设计入口：[受控退役设计](../superpowers/specs/2026-09-11-workitem-controlled-retirement-design.md)。设计经审阅修订，维护者已授权进入[实施计划](../superpowers/plans/2026-09-11-workitem-controlled-retirement.md)。现已实现 P/R 控制入口及三时点隔离验证；Tasks 1–6 分批审阅已通过；最终审阅 I1–I4 修复及限定复审待完成，目标环境执行仍未授权或实施。
...
Tasks 1–6 已通过各批独立审阅，Task 6 包含恢复运行器（6a）、测试准备／文档（6b）和 Redis 恢复修订；最终审阅 I1–I4 修复及限定复审待完成，目标环境执行仍未授权或实施。
...
修订隔离运行已重新完成三个阶段各 9/9，共 27/27，零 SKIP／失败／flaky；原 Redis 停止后，独立 Redis 的实际消费者连接、新登录／刷新与旧令牌拒绝均通过。10 个实际内容／配置损坏反例拒绝；独立 Redis／配置专测 7/7。生产二进制哈希保持上述值。最终自有 9 容器／9 匿名卷及 11 端口均核验清理；Tasks 1–6 分批审阅已通过；最终审阅 I1–I4 修复及限定复审待完成，目标环境执行仍未授权或实施，不能据此宣布目标环境退役完成。

### 2026-09-11 最终累计审阅状态

最终审阅已完成并提出四项必须处理的契约缺口：R 后规范约束重建导致指纹漂移；P 对现有可变流程行的整行摘要阻断正常观察期进展；历史 022 删除清单矛盾校验不完整；generic 身份遗漏专业保留别名。当前处于同一批修复，修复后的针对性结果与新二进制验证不自动构成独立复审通过。

流程基线 I2 的证据边界仍待范围决策：现有整行摘要不能推导新的不可变字段摘要，不能改写旧 P/R 附件、补造历史审计或默认缩减既有 BPMN 拓扑。旧 P 中发生了无法解释的原有流程行变化时继续拒绝 R，后续支持方案须明确协议兼容和独立运维恢复边界。以上不是目标环境退役操作授权。
