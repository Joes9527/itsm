...
后续设计入口：[受控退役设计](../superpowers/specs/2026-09-11-workitem-controlled-retirement-design.md)。设计经审阅修订，维护者已授权进入[实施计划](../superpowers/plans/2026-09-11-workitem-controlled-retirement.md)。现已实现 P/R 控制入口及三时点隔离验证；Tasks 1–6 分批审阅已通过；最终审阅 I1/I3/I4/M1 已修复、限定复审已完成（接手方复审 PASS，非独立第三方）；I2 已按维护者决定列为 backlog，一般 P 前活动流程退役仍未验收。当前 HEAD 隔离全流程重跑 `workitem-v1-e27375881582` 三时点各 9/9、共 27/27，覆盖 observation、R(038)、独立 Redis 恢复与业务 V1；共享 dev 容器 `itsm-postgres-dev` 专属 DB `workitem-target-20260912102644-8d3920` 已完成真实 CLI 准入（037 fail-closed）、P(037) 与普通迁移并精确清理。真实目标环境部署/退役仍未授权或实施；共享目标完整 P→观察→R→恢复与 focused real-PG 未执行。


## 完整恢复与三时点隔离验证入口

目标环境逐步执行、权限核验、写入者/外部投递盘点与证据模板见[受控退役目标环境 Runbook](./workitem-controlled-retirement-target-runbook.md)（待授权执行）。
...
Tasks 1–6 已通过各批独立审阅，Task 6 包含恢复运行器（6a）、测试准备／文档（6b）和 Redis 恢复修订；最终审阅 I1/I3/I4/M1 已修复、限定复审已完成（接手方复审 PASS，非独立第三方）；I2 已按维护者决定列为 backlog，一般 P 前活动流程退役仍未验收。当前 HEAD 隔离全流程重跑 `workitem-v1-e27375881582` 三时点各 9/9、共 27/27，覆盖 observation、R(038)、独立 Redis 恢复与业务 V1；共享 dev 容器 `itsm-postgres-dev` 专属 DB `workitem-target-20260912102644-8d3920` 已完成真实 CLI 准入（037 fail-closed）、P(037) 与普通迁移并精确清理。真实目标环境部署/退役仍未授权或实施；共享目标完整 P→观察→R→恢复与 focused real-PG 未执行。
...
修订隔离运行已重新完成三个阶段各 9/9，共 27/27，零 SKIP／失败／flaky；原 Redis 停止后，独立 Redis 的实际消费者连接、新登录／刷新与旧令牌拒绝均通过。10 个实际内容／配置损坏反例拒绝；独立 Redis／配置专测 7/7。生产二进制哈希保持上述值。最终自有 9 容器／9 匿名卷及 11 端口均核验清理；Tasks 1–6 分批审阅已通过；最终审阅 I1/I3/I4/M1 已修复、限定复审已完成（接手方复审 PASS，非独立第三方）；I2 已按维护者决定列为 backlog，一般 P 前活动流程退役仍未验收。当前 HEAD 隔离全流程重跑 `workitem-v1-e27375881582` 三时点各 9/9、共 27/27，覆盖 observation、R(038)、独立 Redis 恢复与业务 V1；共享 dev 容器 `itsm-postgres-dev` 专属 DB `workitem-target-20260912102644-8d3920` 已完成真实 CLI 准入（037 fail-closed）、P(037) 与普通迁移并精确清理。真实目标环境部署/退役仍未授权或实施；共享目标完整 P→观察→R→恢复与 focused real-PG 未执行。不能据此宣布目标环境退役完成。

### 2026-09-11 最终累计审阅状态

最终审阅已完成并提出四项必须处理的契约缺口：R 后规范约束重建导致指纹漂移；P 对现有可变流程行的整行摘要阻断正常观察期进展；历史 022 删除清单矛盾校验不完整；generic 身份遗漏专业保留别名。当前同一批修复已处理 I1/I3/I4；I2 按 2026-09-12 维护者决定延后。针对性结果与新二进制验证结合接手方限定复审 PASS（非独立第三方）构成当前范围复审结论；严格独立第三方复审如需要仍应另行安排。

2026-09-12 当前 HEAD 隔离全流程重跑：`workitem-v1-e27375881582`（source `a0c5b81a`，base port 19930）三时点各 9/9、共 27/27，覆盖 observation、R(038)、独立 Redis 恢复与业务 V1；28 个 negativeCases；9 容器/9 卷精确清理。共享 dev 容器 `itsm-postgres-dev` 专属 DB `workitem-target-20260912102644-8d3920` 完成真实 CLI 准入（037 fail-closed）、P(037)、普通迁移，ledger 037 存在、038 pending_manual，执行后专属 DB/角色已删除，现有 `itsm` 等库与共享 Redis/MinIO 未被改动，临时 SUPERUSER 已回收。真实生产目标部署/退役仍未授权、未执行；共享目标完整 P→观察→R→恢复与 focused real-PG 未执行。

流程基线 I2 已按维护者“作为 backlog，保证架构，功能优先”决定进入 `BL-WI-PROCESS-AUDIT-CONTINUITY`；权威范围和未来验收见[设计中的 2026-09-12 决定](../superpowers/specs/2026-09-11-workitem-controlled-retirement-design.md)。正常 BPMN 继续运行；P 时既有流程／任务／回调行发生正常变化仍可能导致 R 严格拒绝，刷新备份或签名不能消除原始基线差异。一般在途流程退役能力未验收。本轮不改写旧证据、补造历史审计、缩窄流程或新增功能禁用；不能把该 backlog 计入修复通过项。

规范约束协调使用私有临时表解析当前列类型下的 CHECK 表达式。执行该运维路径的连接须具有数据库 TEMP 权限；不向运行时只读 inspection role 增加任何权限。当前隔离恢复运行使用真实运维连接执行并核验该路径，目标环境仍须独立检查运维权限。
