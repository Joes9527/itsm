...
EmailService.DescribeDeliveryTarget现统一生成v2 EmailTarget（transport、精确Graph连接器身份、digest），参数要求原*ent.Tx，standard Graph沿tx.Client()读取，candidate Graph沿冻结声明；不查询live GraphProvider。可信email_delivery.transport（环境ITSM_EMAIL_DELIVERY_TRANSPORT）只允许graph/smtp，空默认graph，经bootstrap复制到服务；选择smtp不依赖GraphProvider是否存在，Graph描述失败不回退。SMTP摘要使用实际Host/Port/Username/From与tcp机会式STARTTLS/TLS1.2证书验证/PlainAuth，不含Password、不假装SMTP是connector。新配置尚不改变SendForTenant或队列实际路由；原通知生产者/worker现已接入v2；Incident持久协议仍待接入，不能据此启用企业发送。
