...
通知044目标协议已在EnqueueNotificationTx、EnqueueCreationTx和原Worker的连接器分支接入。新意图按当前同渠道唯一实例解析并写入精确目标，重放只复用原身份；Worker发送前后经冻结策略解析并核验generation。该步骤尚未覆盖TicketWorkflowService与BPMN CC直接队列写入及同步SendNotification；相关标准回归当前失败，禁止作为完整通知投递或候选启动放行依据。无目标历史外发意图不得通过回填字段来恢复，后续必须按原owner协议显式处理。
