...
The traceable validation clone has not been created. A differently named database requires a controlled admission extension; an independently hosted restore preserving the existing logical identity is a smaller alternative pending the maintainer's topology choice. This proposal means a second PostgreSQL container on the same WSL host, not another application stack or machine; the final target remains two long-lived ITSM business databases. It changes the original no-additional-PG-instance constraint and has not been approved; see the [proposed topology amendment](superpowers/specs/2026-09-16-dev-restoration-two-database-design.md#克隆部署调整建议proposed尚未执行). Historical comparison databases remain until cleanup gates pass. Exact artifacts, actions and remaining gates are maintained only in the [execution ledger](superpowers/plans/2026-09-15-migration-validation-ledger.md#two-database-execution).
