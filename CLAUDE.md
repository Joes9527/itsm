...
> Accepted controlled retirement design (code and isolated validation implemented; Tasks 1–6 reviewed; I1/I3/I4 fixed pending scoped rereview; I2 deferred as BL-WI-PROCESS-AUDIT-CONTINUITY, so general pre-P active-process retirement remains unaccepted): [design](docs/superpowers/specs/2026-09-11-workitem-controlled-retirement-design.md); [plan](docs/superpowers/plans/2026-09-11-workitem-controlled-retirement.md). Preserve historical SQL/checksums and truthful receipts. Separate transactional structure preparation from later full business acceptance and controlled retirement. All migration write paths, including rollback/reset, enforce stage dependencies; read-only classification precedes any bootstrap writes. Environment deployment and deletion require their own authorization.
 (Summary)
...
- Professional extension tables own domain-specific fields only. **Do not** copy `title`, `description`, `status`, `priority`, `assignee`, `tenant`, `creator`, or public timestamps into an extension table. This drifted once (`ent/schema/incident.go` duplicated `title`/`description`/`status`/`priority`) — resolved on 2026-09-01 by migration `022_drop_professional_extension_shared_fields` (commit `8d71b4f0`), which dropped those columns from `incidents` (and more, e.g. `assignee_id`/`category`/`subcategory`/`source`). Verified clean as of 2026-09-02: `ent/schema/incident.go` and `ent/schema/change.go` carry no such fields. Treat any reappearance of these fields on a professional extension as a regression, not the pre-existing baseline.
