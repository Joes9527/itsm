...
class DepartmentsCheck:
    ...
    def fetch_target(self, target, spec: EntitySpec) -> list[dict]:
        return target.query_rows(
            "SELECT d.code, coalesce(d.name, ''), "
            "CASE WHEN d.parent_id IS NULL THEN '' ELSE coalesce(p.code, '#unresolved-parent') END "
            "FROM %s d LEFT JOIN %s p ON p.id = d.parent_id AND p.tenant_id = d.tenant_id "
            "WHERE %s" % (spec.target_table, spec.target_table, target.scope_predicate('d')),
            ('code', 'name', 'parent_id'))
