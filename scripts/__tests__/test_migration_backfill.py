...
def test_apply_records_created_ids_and_rollback(monkeypatch):
    monkeypatch.setenv('MIGRATED_DEFAULT_PASSWORD', 'pw')
    target = FakeTarget()
    ...
    assert evidence.backfill_results[0]['created_id'] == 101
    assert evidence.backfill_rollback['disable'] == ['PUT /api/v1/users/101/status']
    assert 'DELETE FROM users WHERE id IN (101);' == evidence.backfill_rollback['sql_delete']


def test_apply_refreshes_the_csrf_token_after_each_write(monkeypatch):
    monkeypatch.setenv('MIGRATED_DEFAULT_PASSWORD', 'pw')
    target = FakeTarget()
    run(profile=None, entity_name='users', apply=True, target=target,
        check=FakeCheck([WriteIntent(key='B', payload={'username': 'B'}),
                         WriteIntent(key='C', payload={'username': 'C'})]),
        src=None, tgt=[], evidence=None, spec=spec_with(enabled=True))
    assert [call[2] for call in target.posted] == ['csrf-1', 'csrf-2']
