...
class IsolationTests(unittest.TestCase):
    ...
    def test_artifact_logs_redact_generated_secrets_and_connection_identity(self):
        ...
        config['services']['itsm-backend']['environment'] = {
            ...
            'ITSM_MIGRATION_INSPECTION_DSN': 'postgresql://inspect:inspection-password@postgres/itsm',
            'JWT_SECRET': 'sensitive-signing-key',
        }
        raw = b'sensitive-password postgresql://inspect:inspection-password@postgres/itsm sensitive-signing-key visible failure'
        redacted = ga.redact_logs(config, raw)
        self.assertNotIn('sensitive-password', redacted)
        self.assertNotIn('inspection-password', redacted)
