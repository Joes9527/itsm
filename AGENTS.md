...
## 复杂功能开发经验教训（源自 dynamic-custom-fields 分支实践）

多任务拆解、逐任务评审的开发流程本身不会自动保证质量；以下几条是这条分支实际踩过的坑，作为今后同类工作的硬性检查项：

- **"已完成"的历史结论必须重新验证，不能直接继承。** 设计阶段得出的"这段代码零路由/零调用方"之类结论，到实际执行删除/重构前必须重新跑一遍验证命令，不能假设结论仍然成立——历史调查可能漏看了一条独立的调用链（比如同名但完全不同的第三套实现）。发现结论与现状矛盾时，停下来重新调查、把修正后的结论提给人类确认范围，而不是凭旧结论继续执行。
- **任务级别的代码评审不能替代整分支的最终评审。** 功能拆成多个子任务、每个任务分别评审通过，不代表功能作为整体是对的——跨任务的集成点（比如某任务的后端解析逻辑要跟另一任务的前端提交格式严格对齐）只有在看到完整分支 diff 的最终评审阶段才可能被发现。每个子任务都"合规"，合起来仍可能有严重缺陷。多任务的功能必须有一次覆盖全分支 diff 的最终评审，不能止步于逐任务评审全部通过。
- **修同一类缺陷时，主动搜索代码库里是否有其它地方犯过同样的错误。** 本分支里"字段名带下划线，被前端 http-client 的全局 camelCase 转换悄悄改写、后端精确匹配失败、值被静默丢弃"这个缺陷类别，在同一分支里已经被修过一次（工单模板提交路径），几小时后又在另一条结构相似的路径（服务申请提交）重新引入。修好某个缺陷类别后，应该主动 grep 代码库里结构相似的其它调用点，而不是只改报告里点名的那一处。
- **"清理型"修复本身要经过独立复核，不能拿到 implementer 自报的 DONE 就结束。** 一次修复（比如清理孤儿数据）可能引入新的回归——例如把宿主实体明明是软删除，配的清理逻辑却写成硬删除，导致宿主"恢复"后关联数据永久丢失。这类修复必须再过一轮独立于原实现者的复核，不能只信任第一轮的自我报告。
- **验证方法的"保真度"要讲清楚，不能用低保真度验证悄悄替代高保真度验证。** 没有浏览器自动化工具时，用 API 直接调用（如 curl）走查是合理的替代方案，但必须明确说明它覆盖不到哪些层——比如 curl 天然跳过前端 http-client 的请求体转换逻辑，而缺陷恰好就藏在那一层。API 走查通过，不代表浏览器路径也一定没问题；报告里要老实写清楚哪部分没测到，不能含糊带过。
- **功能"看起来已完成"不等于真的可用，必须走一遍真实使用路径再下结论。** 代码交付、单测/集成测通过，都不能替代一次端到端的真实操作验证；如果环境里没有浏览器自动化工具，至少要用真实 HTTP 客户端路径（而不是内部函数调用）走一遍主链路，并且明确告知用户这次验证覆盖了什么、没覆盖什么。
- **复用旧代码时，要重新审视它当初依赖的假设是否还成立。** 有些旧代码路径是在某个功能"还没真正打通"时写的，其设计假设从来没有被真实数据验证过（例如某个条件分支设计时假设该分支永远不会被触发）。当一个此前半成品的功能第一次被真正打通、有真实数据流过时，依赖它的旧代码需要重新审视，不能假设其设计仍然合理。
