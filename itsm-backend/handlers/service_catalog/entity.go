...
type ServiceCatalog struct {
	...
	Category    string
	Description string
	// TargetClass 是 WorkItem 目标类：service_request_item|incident|change_request，唯一权威
	// 路由依据。调用方（Service.Create/Update）必须显式提供，repository_impl.go 校验后直接落库
	// ——不再从 itsm_type 派生（itsm_type 列已被 migration 024 删除，design doc §7.2）。
	TargetClass    string
...
// IsValidTargetClass 校验调用方提供的 target_class 是否属于受约束的三个取值——Create/Update
// 唯一的校验入口，repository_impl.go 在写入前调用它拒绝空值/未知值（不再有 fail-safe 默认，
// 历史 ComputeTargetClass/itsm_type 派生行为已随 migration 024 一起退役）。
func IsValidTargetClass(targetClass string) bool {
	switch targetClass {
	case TargetClassServiceRequestItem, TargetClassIncident, TargetClassChangeRequest:
		return true
	default:
		return false
	}
