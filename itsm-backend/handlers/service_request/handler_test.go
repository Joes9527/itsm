...
func srSetup(t *testing.T) (*gin.Engine, *ent.Client, int, int, int) {
	...
	cat, err := scSvc.Create(ctx, "SRCatalog-"+srUID(), "software", "for test", 0, tenant.ID, "enabled", 0, 0, nil, "", "", service_catalog.TargetClassServiceRequestItem)
	require.NoError(t, err)
...
func TestServiceRequestHandler_Create_MissingComplianceAck(t *testing.T) {
	...
	scRepo := service_catalog.NewEntRepository(client)
	scService := service_catalog.NewService(scRepo, client, zaptest.NewLogger(t).Sugar())
	infraCat, err := scService.Create(ctx, "VM-"+srUID(), "infrastructure", "for test", 0, tenantID, "enabled", 0, 0, nil, "", "vm", service_catalog.TargetClassServiceRequestItem)
	require.NoError(t, err)
...
func TestServiceRequestCreateDefersNewCIUntilProvisioning(t *testing.T) {
	...
	catalog, err := service_catalog.NewService(scRepo, client, logger).
		Create(ctx, "VM Request", "infrastructure", "Provision VM", 24, tenant.ID, "enabled", ciType.ID, 0, nil, "", "", service_catalog.TargetClassServiceRequestItem)
	require.NoError(t, err)
	expireAt := time.Now().Add(30 * 24 * time.Hour)

	result, err := newServiceRequestIntakeForTest(t, client).Create(ctx, intake.Identity{
		TenantID: tenant.ID, ActorID: user.ID, RequesterID: user.ID, Role: user.Role, Channel: "test",
	}, intake.CreateWorkItemCommand{
		IdempotencyKey: "deferred-ci-" + srUID(), IntakeKind: intake.IntakeKindCatalogItem,
		Title: "Production VM", CatalogItemID: &catalog.ID,
		FormValues: map[string]any{"compliance_ack": true, "data_classification": "internal", "expire_at": expireAt.Format(time.RFC3339)},
	})
	require.NoError(t, err)
	created, err := NewEntRepository(client).Get(ctx, result.ProfessionalReference.ID, tenant.ID)
	require.NoError(t, err)
