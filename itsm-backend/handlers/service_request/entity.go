...
// target_class 是路由的唯一权威来源（design doc §7.2）——handlers/service_catalog 在
// ServiceCatalog 创建/更新时要求调用方显式提供并校验这个字段（历史 itsm_type 列已被
// migration 024_service_catalog_target_class_authority 删除），这里不再重新读取或解释
// itsm_type。
...
func mapTargetClassToTicketType(targetClass string) string {
	...
	case service_catalog.TargetClassIncident:
		return "incident"
	default:
		return "service_request" // service_request_item 及防御性兜底
	}
}
