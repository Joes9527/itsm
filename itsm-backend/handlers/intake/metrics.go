...
func createMetricResult(result *workitemcreation.CreateWorkItemResult, err error) (recordClass, outcome string) {
	if err != nil {
		if errors.Is(err, workitemcreation.ErrIdempotencyConflict) {
...
func boundedMetricRecordClass(value string) string {
	if workitemcreation.IsSupportedRecordClass(value) {
		return value
	}
