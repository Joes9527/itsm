...
SET SESSION app.current_tenant = 999;
UPDATE changes SET risk_level = 'critical' WHERE work_item_id = :same_work_item_id;
-- 预期：UPDATE 0

SET SESSION app.current_tenant = 1;
SELECT risk_level FROM changes WHERE work_item_id = :same_work_item_id;
-- 预期：仍然是 'low'

\echo
\echo === 场景 8：DELETE 跨租户被 USING 拦截 ===
SET SESSION app.current_tenant = 999;
DELETE FROM changes WHERE work_item_id = :same_work_item_id;
-- 预期：DELETE 0

SET SESSION app.current_tenant = 1;
SELECT COUNT(*) AS probe_still_present FROM changes WHERE work_item_id = :same_work_item_id;
-- 预期：1

\echo
\echo === 清理测试探针 ===
DELETE FROM changes WHERE work_item_id = :same_work_item_id;
SELECT COUNT(*) AS after_cleanup FROM changes WHERE work_item_id IN (:same_work_item_id, :cross_work_item_id);
