...
func TestRequestedItemAssignedCollaborationPersistsAndStopsAfterReassignment(t *testing.T) {
	...
	router.POST("/tickets/:id/comments", middleware.RequireWorkItemRecordClassPermission("create"), comments.CreateTicketComment)
	router.PUT("/tickets/:id/comments/:comment_id", middleware.RequireWorkItemRecordClassPermission("update"), comments.UpdateTicketComment)
	router.POST("/tickets/:id/attachments", middleware.RequireWorkItemRecordClassPermission("create"), attachments.UploadAttachment)
