...
func TestBPMNAssignmentSourceMigrationEnforcesImmutabilityAndExactDefault(t *testing.T) {
	...
	require.NoError(t, execBPMNAssignmentSourceMigrationAsset(ctx, db, "047_bpmn_assignment_source.sql"))
	require.NoError(t, execBPMNAssignmentSourceMigrationAsset(ctx, db, "047_bpmn_assignment_source_verify.sql"))
	...
	require.NoError(t, err)
	err = execBPMNAssignmentSourceMigrationAsset(ctx, db, "047_bpmn_assignment_source_verify.sql")
	...
	require.NoError(t, err)
	require.ErrorContains(t, execBPMNAssignmentSourceMigrationAsset(ctx, db, "047_bpmn_assignment_source_verify.sql"), "NOT NULL")
