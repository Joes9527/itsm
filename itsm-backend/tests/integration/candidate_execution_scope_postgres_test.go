...
GRANT SELECT ON execution_scopes,execution_scope_members,execution_runtime_bindings TO %s,%s`, runtimeRole, unboundRole, runtimeRole, unboundRole, runtimeRole, unboundRole, runtimeRole, unboundRole))
	...
	t.Run("runtime admission checks role and configured scopes", func(t *testing.T) {
		...
		_, err = owner.Exec(migration.GetMigrationSQL(migration.ToolExecutionAuthorityLockVersion) + migration.GetMigrationSQL(migration.ToolExecutionAuthorizationLockVersion))
		require.NoError(t, err)
		_, err = owner.Exec("GRANT EXECUTE ON FUNCTION public.lock_candidate_tool_authorization(uuid,text,bigint,bigint,bigint) TO " + runtimeRole)
		require.NoError(t, err)
		...
		t.Run("missing authority capability rejected", func(t *testing.T) {
			_, err := owner.Exec("REVOKE EXECUTE ON FUNCTION public.lock_candidate_tool_authorization(uuid,text,bigint,bigint,bigint) FROM " + runtimeRole)
			require.NoError(t, err)
			require.Error(t, database.ValidateExecutionRuntime(ctx, run, cfg))
			_, err = owner.Exec("GRANT EXECUTE ON FUNCTION public.lock_candidate_tool_authorization(uuid,text,bigint,bigint,bigint) TO " + runtimeRole)
			require.NoError(t, err)
