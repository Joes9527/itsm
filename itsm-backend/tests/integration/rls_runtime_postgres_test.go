...
func TestPostgresRLSRuntimeAuthenticationAndTenantLookup(t *testing.T) {
	...
	svc := authcommon.NewService(authcommon.NewEntRepository(client), "runtime-secret", zap.NewNop().Sugar(), clients.System, refresh, nil)
...
func TestPostgresRLSRuntimeMSPSession(t *testing.T) {
	...
	common := authcommon.NewService(authcommon.NewEntRepository(clients.Tenant), "msp-runtime", zap.NewNop().Sugar(), clients.System, authentication.NewRefreshTokenConsumer("msp-runtime", authentication.NewRedisRefreshTokenStore(redisClient)), nil)
