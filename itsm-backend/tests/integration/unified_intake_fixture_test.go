...
func newUnifiedIntakeFixture(t *testing.T, ticketOwners ...func(*ent.Client, *zap.SugaredLogger) *service.TicketService) *unifiedIntakeFixture {
	...
	for _, owner := range []creation.ProfessionalCreator{genericOwner, service.NewIncidentService(client, logger, executionfixture.Standard()), problemdomain.NewService(nil, logger, executionfixture.Standard()), changedomain.NewService(nil, client, logger), requestdomain.NewService(nil, client, logger, service.NewApprovalChainResolver(client, logger))} {
