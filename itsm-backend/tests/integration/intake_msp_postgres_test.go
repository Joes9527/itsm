...
func TestPostgresIncidentConversionSignedMSPHTTPAuthorizationAndReplay(t *testing.T) {
	...
	foreignItem := f.client.Ticket.Create().SetTenantID(foreignTenant.ID).SetRequesterID(foreignRequester.ID).SetOpenedByID(foreignRequester.ID).SetTitle("Foreign source").SetTicketNumber("INC-FOREIGN").SetRecordClass("incident").SetType("incident").SetStatus("new").SetPriority("high").SaveX(f.ctx)
	foreignIncident := f.client.Incident.Create().SetWorkItemID(foreignItem.ID).SetIncidentNumber("INC-FOREIGN").SetSeverity("high").SetDetectedAt(time.Now()).SaveX(f.ctx)
	...
	deletedItem := f.client.Ticket.Create().SetTenantID(f.tenant.ID).SetRequesterID(requester.ID).SetOpenedByID(requester.ID).SetTitle("Deleted source").SetTicketNumber("INC-DELETED").SetRecordClass("incident").SetType("incident").SetStatus("new").SetPriority("high").SetDeletedAt(time.Now()).SaveX(f.ctx)
	deletedIncident := f.client.Incident.Create().SetWorkItemID(deletedItem.ID).SetIncidentNumber("INC-DELETED").SetSeverity("high").SetDetectedAt(time.Now()).SaveX(f.ctx)
	...
	finalItem := f.client.Ticket.Create().SetTenantID(f.tenant.ID).SetRequesterID(requester.ID).SetOpenedByID(requester.ID).SetTitle("Closed source").SetTicketNumber("INC-CLOSED").SetRecordClass("incident").SetType("incident").SetStatus("closed").SetPriority("high").SaveX(f.ctx)
	finalIncident := f.client.Incident.Create().SetWorkItemID(finalItem.ID).SetIncidentNumber("INC-CLOSED").SetSeverity("high").SetDetectedAt(time.Now()).SaveX(f.ctx)
