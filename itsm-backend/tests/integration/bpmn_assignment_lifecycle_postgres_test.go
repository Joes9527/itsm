...
func newBoundLifecycleFixture(t *testing.T) *boundLifecycleFixture {
	...
	instance := f.client.ProcessInstance.Create().SetProcessInstanceID("bound-instance").SetProcessDefinitionID(definition.ID).SetProcessDefinitionKey(definition.Key).SetBusinessType("generic").SetBusinessID(item.ID).SetStatus("running").SetCurrentActivityID("fulfill").SetTenantID(f.tenant.ID).SaveX(f.ctx)
...
func TestPostgresBoundLifecycleCreationWinsAssignment(t *testing.T) {
	...
	go func() {
		_, err := f.engine.StartProcess(f.scope(f.owner), f.definition.Key, fmt.Sprintf("generic:%d", f.item.ID), "generic", f.item.ID, nil)
		created <- err
	}()
	select {
	case <-entered:
...
func TestPostgresBoundLifecycleAssignmentWinsCreation(t *testing.T) {
	...
	if creationErr != nil {
		...
		require.ErrorAs(t, creationErr, &conflict)
		require.Equal(t, pq.ErrorCode("40001"), conflict.Code)
		require.Equal(t, 1, f.client.ProcessTask.Query().CountX(f.ctx), "failed RR creation rolls back fully")
		_, retryErr := f.engine.StartProcess(f.scope(f.owner), f.definition.Key, fmt.Sprintf("generic:%d", f.item.ID), "generic", f.item.ID, nil)
	...
	for _, kind := range []string{"service_task", "user_task_callback"} {
		for _, first := range []string{"callback", "assignment"} {
			t.Run(kind+"/"+first, func(t *testing.T) {
				f := newBoundLifecycleFixture(t)
				var sawSerialization atomic.Bool
				...
				advanceErr := <-advanced
				if advanceErr != nil {
					require.Equal(t, "assignment", first, "callback-first must complete under its held lock")
					require.True(t, sawSerialization.Load(), "pending callback retry must be caused by observed PostgreSQL 40001")
					durable := f.client.ProcessCallbackOutbox.GetX(f.ctx, row.ID)
					require.Equal(t, "pending", durable.Status, "RR conflict must remain retryable")
					require.Equal(t, 1, f.client.ProcessTask.Query().Where(processtask.AssigneeSourceEQ("work_item_assignee")).CountX(f.ctx), "rolled-back advance cannot leave partial tasks")
					// Worker owns durable retry after the competing commit, with a fresh RR snapshot.
					f.setup.client.ProcessCallbackOutbox.UpdateOneID(row.ID).SetNextAttemptAt(time.Now().Add(-time.Second)).ExecX(f.setup.ctx)
					count, retryErr := f.engine.ProcessPendingCallbacks(f.ctx, "race-retry-worker", 10)
					require.NoError(t, retryErr)
...
func TestPostgresBoundLifecycleReadSnapshotAndIndependentProof(t *testing.T) {
	for _, bound := range []bool{false, true} {
		t.Run(fmt.Sprint(bound), func(t *testing.T) {
			...
			f.engine.SetDirectorySnapshot(directory)
			if !bound {
...
func TestPostgresBoundTaskDetailKeepsOneCompletionSnapshot(t *testing.T) {
	f := newBoundLifecycleFixture(t)
	directory := &countingTaskReadDirectory{DirectorySnapshot: f.runtime.IntakeDirectorySnapshot()}
	f.engine.SetDirectorySnapshot(directory)
	changed := false
	f.client.ProcessTask.Intercept(ent.InterceptFunc(func(next ent.Querier) ent.Querier {
		return ent.QuerierFunc(func(ctx context.Context, q ent.Query) (ent.Value, error) {
			...
			if err == nil && !changed {
				changed = true
				require.NoError(t, f.engine.CompleteTask(f.scope(f.owner), f.task.TaskID, nil))
				require.NoError(t, f.assign(f.ctx, f.next.ID, nil))
			}
			return value, err
		})
	}))
	before, err := f.engine.TaskService().GetTaskView(f.scope(f.admin), f.task.TaskID)
	require.NoError(t, err)
	require.True(t, changed)
	require.Equal(t, "created", before.Status)
	require.Equal(t, f.owner.ID, before.ResponsibleUserID)
	require.Equal(t, "assigned", before.AssignmentState)
	require.True(t, before.UIActions.Complete)
	after, err := f.engine.TaskService().GetTaskView(f.scope(f.admin), f.task.TaskID)
	require.NoError(t, err)
	require.Equal(t, "completed", after.Status)
	require.Equal(t, "terminal", after.AssignmentState)
	require.Equal(t, f.owner.ID, after.ResponsibleUserID)
	require.Equal(t, f.owner.ID, after.ActorID)
	require.False(t, after.UIActions.Complete)
	require.Equal(t, f.next.ID, f.client.Ticket.GetX(f.ctx, f.item.ID).AssigneeID, "live owner changed, terminal responsibility remains frozen")
}
