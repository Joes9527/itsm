...
func TestTicketTypesInitializationRealCLIPlanAndApply(t *testing.T) {
	...
	payload, err := json.Marshal(map[string]interface{}{"database": map[string]interface{}{
		...
		"sslmode": "disable", "schema": u.Query().Get("search_path")}, "deployment": map[string]interface{}{"mode": "private", "auto_migrate": false, "auto_seed": false}})
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(filepath.Join(dir, "config.yaml"), payload, 0600))
