...
func TestCandidateConstructPreservesDatabaseAndStreams(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = owner.ExecContext(ctx, migration.GetMigrationSQL(migration.ToolExecutionAuthorityLockVersion)+migration.GetMigrationSQL(migration.ToolExecutionAuthorizationLockVersion))
	require.NoError(t, err)
	_, err = owner.ExecContext(ctx, "GRANT EXECUTE ON FUNCTION public.lock_candidate_tool_authorization(uuid,text,bigint,bigint,bigint) TO "+runRole)
	require.NoError(t, err)
