...
func TestPostgresWorkItemAssignmentWorkerRetryRestartAndUnknownTransport(t *testing.T) {
	...
	email := assignmentGraphTargetFixture(t, client, ctx, cmd.TenantID, executionfixture.Standard("notification"), graph)
	notifications := service.NewTicketNotificationService(client, zap.NewNop().Sugar(), executionfixture.Standard("notification"))
	...
	newWorker := func() *service.OutboxDeliveryWorker {
		...
		require.NoError(t, err)
		worker, err := service.NewOutboxDeliveryWorker(service.NewOutboxEventRepository(client, executionfixture.Standard("outbox")), service.OutboxDeliveryWorkerConfig{BatchSize: 10, PollInterval: time.Second, HandlerTimeout: time.Minute, MaxAttempts: 3}, zap.NewNop().Sugar(), registry)
		require.NoError(t, err)
		return worker
	}
	require.NoError(t, newWorker().DispatchOnce(context.Background()))
...
func TestPostgresWorkItemAssignmentSuppressedDeliverySurvivesPreferenceChangeAndRestart(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, tx.Commit())
	repository := service.NewOutboxEventRepository(client, executionfixture.Standard("outbox"))
	claimed, err := repository.ClaimDue(context.Background(), time.Now().Add(time.Second), 1)
	require.NoError(t, err)
...
func TestPostgresWorkItemAssignmentCapturesOnlyLiveBoundTasksInStableOrder(t *testing.T) {
	...
	instance := client.ProcessInstance.Create().SetTenantID(cmd.TenantID).SetProcessInstanceID("instance").SetProcessDefinitionID(definition.ID).SetProcessDefinitionKey(definition.Key).SetBusinessID(cmd.WorkItemID).SetBusinessType("generic").SaveX(ctx)
	expected := []int{}
...
func TestPostgresWorkItemAssignmentAtomicAndIdempotent(t *testing.T) {
	...
	notifications := service.NewTicketNotificationService(client, zap.NewNop().Sugar(), executionfixture.Standard("notification"))
	notifications.SetAssignmentDirectory(sameTransactionDirectory{})
	notifications.SetEmailService(assignmentGraphTargetFixture(t, client, ctx, cmd.TenantID, executionfixture.Standard("notification"), &assignmentMailRecorder{}))
	for i := 0; i < 2; i++ {
		require.NoError(t, service.NewWorkItemAssignmentNotificationHandler(client, notifications).Deliver(ctx, event))
	}
