...
func runtimeClients(t *testing.T, f *incidentEffectsFixture) (*database.RuntimeClients, config.DatabaseConfig) {
	...
	for _, table := range []string{"tickets", "incidents", "incident_rules", "incident_rule_executions", "incident_rule_action_receipts", "incident_metrics", "incident_alerts", "notifications", "outbox_events", "audit_logs", "intake_requests", "users", "tenants", "roles", "permissions", "role_permissions", "incident_events", "ticket_categories"} {
	...
	for _, grant := range []string{"SELECT ON users,tenants,msp_allocations,connector_configs", "SELECT,UPDATE ON outbox_events", "INSERT,SELECT(id) ON audit_logs", "USAGE ON SEQUENCE audit_logs_id_seq"} {
