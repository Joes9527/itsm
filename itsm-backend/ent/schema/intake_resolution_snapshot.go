...
func (IntakeResolutionSnapshot) Edges() []ent.Edge {
	return []ent.Edge{
		edge.To("receipt", IntakeRequest.Type).Field("intake_request_id").Unique().Required().Immutable(),
		edge.To("work_item", Ticket.Type).Field("work_item_id").Unique().Required().Immutable(),
	}
}
