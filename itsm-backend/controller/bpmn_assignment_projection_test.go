...
func TestBPMNBoundTaskDetailUsesAssignmentDTO(t *testing.T) {
	...
	instance := client.ProcessInstance.Create().SetProcessInstanceID("bound").SetProcessDefinitionID(definition.ID).SetProcessDefinitionKey(definition.Key).SetBusinessType("service_request").SetBusinessID(item.ID).SetTenantID(tenant.ID).SaveX(ctx)
