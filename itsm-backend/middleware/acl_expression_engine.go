...
// ACLScriptContext 提供 ACL 脚本执行所需的上下文信息
type ACLScriptContext struct {
	...
	ResourceType string
	ResourceOwnerID interface{}
}

// EvaluateACLScript 根据脚本上下文评估 ACL 脚本
func EvaluateACLScript(script string, ctx ACLScriptContext) bool {
	// 将上下文转换为变量映射
	variables := map[string]interface{}{}

	if ctx.UserID != nil {
		variables["ctx.user_id"] = ctx.UserID
	}
	if ctx.TenantID != nil {
		variables["ctx.tenant_id"] = ctx.TenantID
	}
	if ctx.Role != "" {
		variables["ctx.role"] = ctx.Role
	}
	if ctx.ResourceID != nil {
		variables["ctx.resource_id"] = ctx.ResourceID
	}
	if ctx.ResourceType != "" {
		variables["ctx.resource_type"] = ctx.ResourceType
	}
	if ctx.ResourceOwnerID != nil {
		variables["ctx.resource.owner_id"] = ctx.ResourceOwnerID
	}

	// 使用引擎评估脚本
	engine := NewACLExpressionEngine()
	return engine.Evaluate(script, variables)
}
