...
func (s *KafDelegationService) ExecuteAction(ctx context.Context, taskID string, req KafActionRequest, engine ProcessEngine) (*KafActionResult, error) {
	...
	if req.Action == kafActionComplete {
		...
		variables["kaf_execution"] = map[string]interface{}{
...
func (s *KafDelegationService) CreateDelegatedTask(ctx context.Context, instanceID int, serviceTask *BPMNServiceTask) (*ent.ProcessTask, error) {
	...
	event, err := s.newDelegatedTaskOutboxEvent(ctx, tx.Client(), task)
	if err != nil {
		return nil, err
	}
...
func (s *KafDelegationService) CreateDelegatedTaskWithClient(ctx context.Context, client *ent.Client, instanceID int, serviceTask *BPMNServiceTask) (*ent.ProcessTask, error) {
	...
	event, err := s.newDelegatedTaskOutboxEvent(ctx, client, task)
	if err != nil {
		return nil, err
	}
...
func (s *KafDelegationService) createDelegatedTaskWithClient(ctx context.Context, client *ent.Client, instanceID int, serviceTask *BPMNServiceTask) (*ent.ProcessTask, error) {
	...
	actorID, _ := ctx.Value(bpmn.BPMNUserIDContextKey).(int)
	if actorID > 0 {
		actorExists, err := client.User.Query().
	...
	task, err := client.ProcessTask.Create().
		...
		SetCallbackHandlerID("kaf_delegate_handler").
		SetCallbackTaskType(bpmn.KafDelegateTaskType).
		SetCallbackAction(serviceTask.ServiceTaskAction()).
		SetCallbackConfigRef(serviceTask.CallbackConfigRef()).
...
func (s *KafDelegationService) newDelegatedTaskOutboxEvent(ctx context.Context, client *ent.Client, task *ent.ProcessTask) (NewOutboxEvent, error) {
	instance, err := client.ProcessInstance.Query().Where(
		processinstance.IDEQ(task.ProcessInstanceID), processinstance.TenantIDEQ(task.TenantID),
	).Only(ctx)
	if err != nil {
		return NewOutboxEvent{}, fmt.Errorf("load KAF delegation process instance for outbox: %w", err)
	}
	return newKafDelegateOutboxEvent(ctx, client, task, instance, s.now())
}
