...
func EnqueueWorkItemAssignment(ctx context.Context, client *ent.Client, event assignment.Event) error {
	...
	_, err = enqueueOutboxEvent(ctx, client, nil, NewOutboxEvent{ExecutionWorkItemID: event.Command.WorkItemID, EventID: event.EventID, EventType: assignment.EventType, TenantID: event.Command.TenantID, AggregateType: "work_item", AggregateID: strconv.Itoa(event.Command.WorkItemID), Payload: payload})
	return err
}
...
func decodeWorkItemAssignment(event *ent.OutboxEvent) (assignment.Event, error) {
	...
	if decoder.Decode(&extra) != io.EOF || p.Command.Validate() != nil || p.PreviousAssigneeID < 0 || p.PreviousAssigneeID == p.Command.AssigneeID || p.Version != p.Command.ExpectedVersion+1 ||
		...
		event.AggregateType != "work_item" || event.AggregateID != strconv.Itoa(p.Command.WorkItemID) || event.ExecutionWorkItemID == nil || *event.ExecutionWorkItemID != p.Command.WorkItemID {
...
func (h *WorkItemAssignmentNotificationHandler) Deliver(ctx context.Context, event *ent.OutboxEvent) error {
	...
	defer tx.Rollback()
	if h.notifications.execution == nil {
		return fmt.Errorf("assignment delivery execution policy required")
	}
	if err := h.notifications.execution.BindEnt(ctx, tx, p.Command.TenantID); err != nil {
		return err
	}
	if err := h.notifications.execution.RequireEntMembers(ctx, tx, p.Command.TenantID, p.Command.WorkItemID); err != nil {
		return err
	}
	...
	repository := NewOutboxEventRepository(tx.Client(), h.notifications.execution)
	...
	if err = h.notifications.enqueueTicketNotificationTx(ctx, tx, item, "ticket_assigned", content, p.EventID, []int{p.Command.AssigneeID}, p.Command.AssigneeID); err != nil {
		return err
	}
	if err = repository.RecordDeliveryCompleted(ctx, event); err != nil {
		return err
