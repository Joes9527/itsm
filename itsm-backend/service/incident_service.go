...
func (s *IncidentService) CreateIncident(ctx context.Context, req *dto.CreateIncidentRequest, tenantID, userID int) (*dto.IncidentResponse, error) {
	...
	if categoryID != nil {
		category, categoryErr := tx.TicketCategory.Query().
			Where(ticketcategory.IDEQ(*categoryID)).
			WithParent().
			Only(ctx)
		if categoryErr != nil {
			return rollback(fmt.Errorf("failed to load incident category projection: %w", categoryErr))
		}
...
func (s *IncidentService) UpdateIncident(ctx context.Context, id int, req *dto.UpdateIncidentRequest, tenantID int) (*dto.IncidentResponse, error) {
	...
	updateQuery := tx.Incident.UpdateOneID(id).Where(incidentTenantScope(tenantID))
	if req.Severity != nil {
	...
	workItemUpdate := tx.Ticket.UpdateOneID(currentIncident.WorkItemID).
...
func (s *IncidentService) EscalateIncident(ctx context.Context, req *dto.IncidentEscalationRequest, tenantID int) (*dto.IncidentEscalationResponse, error) {
	...
	tx, err := s.client.Tx(ctx)
	if err != nil {
		return nil, fmt.Errorf("start incident escalation transaction: %w", err)
	}
	rollback := func(cause error) (*dto.IncidentEscalationResponse, error) {
		_ = tx.Rollback()
		return nil, cause
	}
	...
	if err != nil {
		...
		return rollback(fmt.Errorf("failed to escalate incident: %w", err))
	}
	if err := tx.Commit(); err != nil {
		return rollback(fmt.Errorf("commit incident escalation: %w", err))
	}
	incidentEntity.Edges.WorkItem = workItem
...
func (s *IncidentService) triggerWorkflowForIncident(ctx context.Context, incidentID int, tenantID int) error {
	...
	// 构建流程变量
	incidentResponse := s.toIncidentResponse(inc)
	variables := map[string]interface{}{
		...
		"reporter_id":     inc.Edges.WorkItem.RequesterID,
		"assignee_id":     inc.Edges.WorkItem.AssigneeID,
	}
	...
	triggerReq := &dto.ProcessTriggerRequest{
		...
		Variables:            variables,
		TriggeredBy:          fmt.Sprintf("%d", inc.Edges.WorkItem.RequesterID),
		TriggeredAt:          time.Now(),
		TenantID:             tenantID,
	}
