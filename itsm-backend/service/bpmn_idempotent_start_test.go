...
func TestDefinitionStartReplaysCommittedInstanceAndPinsVersion(t *testing.T) {
	...
	first, err := starter.StartProcessByDefinitionID(ctx, FreezeProcessDefinition(f.definition), "generic:91", "ticket", 91, vars, "workflow-start:91:1")
	...
	second, err := starter.StartProcessByDefinitionID(ctx, FreezeProcessDefinition(f.definition), "generic:91", "ticket", 91, vars, "workflow-start:91:1")
	require.NoError(t, err)
...
func TestDefinitionStartRejectsIdentityConflictAndRevokedActor(t *testing.T) {
	...
	require.True(t, ok, "engine must support durable start identity and exact definition")
	ctx := f.scopedCtx(false, false, false, false)
	_, err := starter.StartProcessByDefinitionID(ctx, FreezeProcessDefinition(f.definition), "generic:91", "ticket", 91, nil, "start:one")
	require.NoError(t, err)
	_, err = starter.StartProcessByDefinitionID(ctx, FreezeProcessDefinition(f.definition), "generic:92", "ticket", 92, nil, "start:one")
	require.Error(t, err)
...
func TestDefinitionStartRejectsChangedContextAfterVariablesAdvance(t *testing.T) {
	...
	first, err := f.engine.StartProcessByDefinitionID(ctx, FreezeProcessDefinition(f.definition), "generic:91", "ticket", 91, vars, "start:context")
	require.NoError(t, err)
	...
	_, err = f.engine.StartProcessByDefinitionID(ctx, FreezeProcessDefinition(f.definition), "generic:91", "ticket", 91, vars, "start:context")
	require.NoError(t, err)
	_, err = f.engine.StartProcessByDefinitionID(ctx, FreezeProcessDefinition(f.definition), "generic:91", "ticket", 91, map[string]interface{}{"title": "changed", "requester_id": f.actor.ID}, "start:context")
...
func TestDefinitionStartReplaysAfterDefinitionContentChanges(t *testing.T) {
	...
	first, err := f.engine.StartProcessByDefinitionID(ctx, frozen, "generic:91", "ticket", 91, nil, "start:edited")
	require.NoError(t, err)
	f.client.ProcessDefinition.UpdateOneID(f.definition.ID).SetBpmnXML(startProcessUserTaskXML()).SetIsActive(false).ExecX(ctx)
	replay, err := f.engine.StartProcessByDefinitionID(ctx, frozen, "generic:91", "ticket", 91, nil, "start:edited")
	require.NoError(t, err)
	require.Equal(t, first.ID, replay.ID)
	_, err = f.engine.StartProcessByDefinitionID(ctx, frozen, "generic:92", "ticket", 92, nil, "start:new")
...
func TestDefinitionStartRejectsLegacyInstanceWithoutOriginalDigest(t *testing.T) {
	...
	first, err := f.engine.StartProcessByDefinitionID(ctx, FreezeProcessDefinition(f.definition), "generic:91", "ticket", 91, nil, "start:legacy")
	require.NoError(t, err)
	require.Empty(t, first.StartRequestDigest)
	_, err = f.engine.StartProcessByDefinitionID(ctx, FreezeProcessDefinition(f.definition), "generic:91", "ticket", 91, nil, "start:legacy")
	require.ErrorContains(t, err, "conflict")
	require.Equal(t, 1, f.client.ProcessInstance.Query().CountX(ctx))
}
...
func TestDefinitionStartRollsBackAutomaticIntentOnAuditFailure(t *testing.T) {
	...
	_, err := f.engine.StartProcessByDefinitionID(startProcessContext(f), FreezeProcessDefinition(definition), "generic:105", "ticket", 105, nil, "start:rollback")
...
func TestDefinitionStartLeavesFailedAutomaticCallbackRecoverable(t *testing.T) {
	...
	configureStartProcessDefinition(t, f, startProcessServiceTaskXML(handler.GetTaskType()))
	definition := f.client.ProcessDefinition.GetX(f.userCtx, f.definition.ID)
	instance, err := f.engine.StartProcessByDefinitionID(startProcessContext(f), FreezeProcessDefinition(definition), "generic:106", "ticket", 106, nil, "start:pending")
	require.NoError(t, err)
...
func TestDefinitionStartRejectsChangedActorDefinitionAndMissingScope(t *testing.T) {
	...
	ctx := startProcessContext(f)
	frozen := FreezeProcessDefinition(f.definition)
	_, err := f.engine.StartProcessByDefinitionID(ctx, frozen, "generic:91", "ticket", 91, nil, "start:identity")
	require.NoError(t, err)
	otherActor := WithBPMNAccessScope(context.Background(), BPMNAccessScope{TenantID: f.tenant.ID, UserID: f.outsider.ID})
	_, err = f.engine.StartProcessByDefinitionID(otherActor, frozen, "generic:91", "ticket", 91, nil, "start:identity")
	require.ErrorContains(t, err, "conflict")
	changed := frozen
	changed.ID++
	_, err = f.engine.StartProcessByDefinitionID(ctx, changed, "generic:91", "ticket", 91, nil, "start:identity")
	require.ErrorContains(t, err, "conflict")
	_, err = f.engine.StartProcessByDefinitionID(context.Background(), frozen, "generic:91", "ticket", 91, nil, "start:identity")
	require.Error(t, err)
	require.Equal(t, 1, f.client.ProcessInstance.Query().CountX(ctx))
}
