...
func TestWorkItemAssignmentNotificationRejectsForgedPayload(t *testing.T) {
	p := assignment.Event{Command: assignment.Command{WorkItemID: 2, TenantID: 1, ActorTenantID: 1, ActorID: 3, AssigneeID: 4, ExpectedVersion: 1, Source: "workflow"}, PreviousAssigneeID: 0, Version: 2, EventID: "work-item-assigned:1:2:2"}
	data, err := json.Marshal(p)
	require.NoError(t, err)
	good := ent.OutboxEvent{EventID: p.EventID, EventType: "work_item.assigned", TenantID: 1, AggregateType: "work_item", AggregateID: "2", Payload: data}
