...
func TestWorkItemAssignmentEventRequiresRecordClass(t *testing.T) {
	for _, recordClass := range []string{"", "unknown", "incident"} {
		t.Run(recordClass, func(t *testing.T) {
			...
			require.NoError(t, err)
			_, err = decodeWorkItemAssignment(&ent.OutboxEvent{EventID: "work-item-assigned:1:2:2", EventType: assignment.EventType, TenantID: 1, AggregateType: "work_item", AggregateID: "2", Payload: data})
