...
type bpmnTaskReadSnapshot struct {
	...
	permissions     map[[2]int][]authorization.Permission
...
func (v *bpmnTaskReadSnapshot) resetChunk() {
	...
	v.permissions = make(map[[2]int][]authorization.Permission)
...
func (e *CustomProcessEngine) withTaskReadSnapshot(ctx context.Context, run func(context.Context, *CustomProcessEngine) error) error {
	...
	return withBPMNTaskReadSnapshot(ctx, e.client, func(ctx context.Context, tx *ent.Tx) error { return run(ctx, e.forClient(tx.Client(), nil, tx)) })
}
