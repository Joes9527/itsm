...
func TestCallbackProvenanceEnqueueAuditRollback(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = outbox.enqueue(ctx, tx.Client(), bpmnCallbackEnqueueRequest{ExecutionKey: "must-rollback", TenantID: 7, ProcessInstanceID: original.ProcessInstanceID, CallbackKind: "service_task", HandlerID: "test", TaskType: "test", ElementID: "activity", Action: "assign_ticket"})
...
func callbackAssignmentTestContext(t *testing.T, client *ent.Client, actor *ent.User, target, workItemID int) context.Context {
	...
	tx, err := client.Tx(ctx)
	require.NoError(t, err)
	_, err = newBPMNCallbackOutboxForTest(client, nil, time.Now()).enqueue(ctx, tx.Client(), bpmnCallbackEnqueueRequest{ExecutionKey: key, TenantID: target, ProcessInstanceID: instance.ID, CallbackKind: "service_task", HandlerID: "test", TaskType: "test", ElementID: "assign", Action: "assign_incident"})
	require.NoError(t, err)
