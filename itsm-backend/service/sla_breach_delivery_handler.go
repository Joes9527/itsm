...
func (e *persistedSLABreachEvent) ExecutionWorkItemID() int {
	id, _ := strconv.Atoi(e.TicketID)
	return id
}
func (e *persistedSLABreachEvent) PersistentEventID() string { return e.EventID }
...
func (*SLABreachDeliveryHandler) Deliver(ctx context.Context, row *ent.OutboxEvent) error {
	...
	if err := bus.Publish(ev); err != nil {
		...
		return blockOutboxDelivery(fmt.Sprintf("delivery_unknown: SLA event %s publish outcome requires reconciliation", ev.EventID))
	}
	return nil
}

func persistedSLABreach(row *ent.OutboxEvent) (*persistedSLABreachEvent, error) {
	if row == nil {
		return nil, blockOutboxDelivery("SLA breach event is required")
	}
	var p slaBreachDeliveryPayload
	if err := json.Unmarshal(row.Payload, &p); err != nil {
		return nil, blockOutboxDelivery("invalid SLA breach payload")
	}
	if p.Version != 1 || p.EventID == "" || p.EventID != row.EventID || row.EventType != slaBreachEventType || p.TenantID <= 0 || p.TenantID != row.TenantID || p.TicketID <= 0 || (row.ExecutionWorkItemID == nil || p.TicketID != *row.ExecutionWorkItemID) || p.ViolationID <= 0 || row.AggregateType != "sla_violation" || row.AggregateID != strconv.Itoa(p.ViolationID) || p.SLAPolicyID <= 0 || p.BreachedAt.IsZero() || (p.BreachedType != "response" && p.BreachedType != "resolve") {
		return nil, blockOutboxDelivery("SLA breach event identity or contract mismatch")
	}
	return &persistedSLABreachEvent{SLABreachedEvent: event.NewSLABreachedEvent(strconv.Itoa(p.TenantID), strconv.Itoa(p.TicketID), strconv.Itoa(p.SLAPolicyID), p.BreachedType, p.BreachedAt), EventID: p.EventID}, nil
}
