...
//   - incident.work_item_id 为空表示开发数据违反 WorkItem 创建不变量；对应评论事件会被
//     跳过并计入 skipped。本工具只搬评论，不修复无效领域记录。
...
func resolvePlan(ctx context.Context, client *ent.Client, event *ent.IncidentEvent) (plan commentPlan, ok bool, reason string, err error) {
	...
	if inc.WorkItemID == 0 {
		return commentPlan{}, false, "incident 违反 WorkItem 不变量：work_item_id 为空", nil
	}
	if event.UserID <= 0 {
...
// 不同跳过原因的运维含义差别很大（比如 work_item_id 为空意味着开发数据违反 WorkItem
// 创建不变量），折叠成一个数字会让操作员看不出该不该先处理无效数据。
//
