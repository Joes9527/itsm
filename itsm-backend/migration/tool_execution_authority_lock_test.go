...
func TestToolAuthorityLockMigrationRequiresPriorPreparation(t *testing.T) {
	catalog := ControlledMigrationCatalog()
	var prefix []Migration
	for _, d := range catalog {
		if d.Migration.Version == ToolExecutionAuthorityLockVersion {
			break
		}
	...
	require.Len(t, plan.Executable, 10)
	...
	for _, removed := range []string{WorkItemPrepareVersion, CandidateExecutionScopeVersion, SLAAlertNotificationVersion, ToolInvocationExecutionScopeVersion} {
		...
		invalid = append(invalid, Migration{Version: ToolExecutionAuthorityLockVersion})
