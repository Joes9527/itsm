...
var RegisteredMigrations = []Migration{
	...
	{
		Version: "018_convert_legacy_serial_ids_to_identity",
		Description: "Convert the 13 tables still on old-style SERIAL/BIGSERIAL id columns " +
			"(change_approval_chains, change_approvals, change_implementation_plans, " +
			"change_risk_assessments, change_rollback_executions, change_rollback_plans, " +
			"initialization_component_attempts, initialization_installations, " +
			"initialization_managed_records, initialization_runs from raw-SQL migrations " +
			"007/008, plus ent-managed ai_feedbacks/connector_configs/vectors " +
			"predating ent's switch to identity-by-default) to GENERATED BY DEFAULT AS IDENTITY, " +
			"matching every other table. Without this, ent's additive auto-migration " +
			"(client.Schema.Create, gated by AutoMigrate) fails atomically the moment atlas " +
			"reaches any of these tables, with 'unexpected attribute change (expect IDENTITY)' " +
			"-- discovered while trying to run cmd/check_work_item_integrity's prerequisite " +
			"schema sync against itsm-postgres-dev for the first time. Preserves existing rows " +
			"and continues each identity sequence from the table's current max(id); skips tables " +
			"that don't exist (e.g. vectors, if RAG isn't provisioned in a given environment) or " +
...
`
	case "018_convert_legacy_serial_ids_to_identity":
		return `
DO $$
DECLARE
    tbl text;
...
BEGIN
    FOREACH tbl IN ARRAY ARRAY[
        'ai_feedbacks', 'change_approval_chains', 'change_approvals',
        'change_implementation_plans', 'change_risk_assessments',
        'change_rollback_executions', 'change_rollback_plans',
        'connector_configs',
        'initialization_component_attempts', 'initialization_installations',
        'initialization_managed_records', 'initialization_runs', 'vectors'
    ]
    LOOP
        ...
        SELECT (attidentity <> ''), (atttypid IN ('int4'::regtype, 'int8'::regtype))
        INTO is_identity, is_int
        FROM pg_attribute
        WHERE attrelid = tbl::regclass AND attname = 'id';

        IF is_identity OR NOT is_int THEN
            CONTINUE;
        END IF;

        EXECUTE format('SELECT COALESCE(MAX(id), 0) + 1 FROM %I', tbl) INTO next_id;
        EXECUTE format('ALTER TABLE %I ALTER COLUMN id DROP DEFAULT', tbl);
        EXECUTE format('ALTER TABLE %I ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (START WITH %s)', tbl, next_id);
...
WHERE execution.change_id = c.id
  AND execution.tenant_id IS DISTINCT FROM wi.tenant_id;

UPDATE change_approval_chains execution
SET tenant_id = wi.tenant_id
FROM changes c
JOIN tickets wi ON wi.id = c.work_item_id
WHERE execution.change_id = c.id
...
UPDATE change_risk_assessments execution
SET tenant_id = wi.tenant_id
FROM changes c
JOIN tickets wi ON wi.id = c.work_item_id
WHERE execution.change_id = c.id
...
UPDATE change_rollback_plans execution
SET tenant_id = wi.tenant_id
FROM changes c
JOIN tickets wi ON wi.id = c.work_item_id
WHERE execution.change_id = c.id
...
UPDATE change_rollback_executions execution
SET tenant_id = wi.tenant_id
FROM changes c
JOIN tickets wi ON wi.id = c.work_item_id
WHERE execution.change_id = c.id
...
UPDATE change_implementation_plans execution
SET tenant_id = wi.tenant_id
FROM changes c
JOIN tickets wi ON wi.id = c.work_item_id
WHERE execution.change_id = c.id
