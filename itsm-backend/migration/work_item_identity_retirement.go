...
// WorkItem identity is authoritative; legacy assertions are checked before removal.
const workItemIdentityRetirementSQL = `
DO $retire$
...
END $retire$;
` + workItemIdentityRetirementVerifySQL

const workItemIdentityRetirementVerifySQL = `
DO $verify$
BEGIN
