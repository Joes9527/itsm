...
The five consecutive ports are backend, frontend, Redis, MinIO and PostgreSQL. The runner refuses occupied ports, creates randomly named and labeled containers, bootstraps the canonical schema and seed, then runs the application with a non-owner, non-superuser, non-BYPASSRLS business role. The narrowly granted system role retains BYPASSRLS for the existing directory/outbox boundary. It builds the current backend and copies current frontend sources to a private temporary directory so no shared Next build output is reused.
