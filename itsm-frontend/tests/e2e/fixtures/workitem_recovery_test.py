...
class RecoveryTests(unittest.TestCase):
    def test_full_recovery_requires_every_surface(self):
        baseline={name:{'sha256':'same'} for name in ['tables','sequences','roles_acl','attachments','application','consumers','configuration','control','config_yaml','identity_provider_config']}
        baseline['redis']={'observedAtMS':100,'databases':16,'keys':{},'families':{k:0 for k in ['jwt:refresh:consumed:','jwt:revoked:','intake:identity-exchange:nonce:','stream','other-preserved']}}
    ...
    def test_missing_redis_inventory_cannot_prove_recovery(self):
        old={name:{'sha256':'same'} for name in ['tables','sequences','roles_acl','attachments','application','consumers','configuration','control','config_yaml','identity_provider_config']}
        with self.assertRaises(ValueError): verify_recovery(old,copy.deepcopy(old))
