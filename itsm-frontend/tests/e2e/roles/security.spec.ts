...
test.describe('US6: security 安全管理员只读审计', () => {
  ...
  test('T047 - 能查看登录日志', async ({ apiGet }) => {
    const response = await apiGet(role, '/api/v1/audit-logs?type=login');
    expect(response.status).toBe(403);
  });

  test('T048 - 安全管理员无用户管理写权限', async ({ apiPostExpectStatus }) => {
    // 尝试创建用户 - 应该被拒绝
    const response = await apiPostExpectStatus(role, '/api/v1/users', 403, {
      username: 'hacker',
      password: 'test',
      name: 'Test',
    });
