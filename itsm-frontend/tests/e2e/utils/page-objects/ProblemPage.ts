...
export class ProblemPage extends BasePage {
  ...
  async createProblem(data: {
    ...
    if (data.rootCause) {
      ...
      const rootCauseInput = this.page
        .locator(
          'textarea[id*="rootCause"], textarea[name*="rootCause"], textarea[placeholder*="根本原因"]'
        )
        .first();
    ...
    // 填写解决方案
    if (data.resolution) {
      const resolutionInput = this.page
        .locator(
          'textarea[id*="resolution"], textarea[name*="resolution"], textarea[placeholder*="解决方案"]'
        )
        .first();
  ...
  async linkIncident(problemId: number, incidentId: number): Promise<void> {
    ...
    await expect(linkButton).toBeVisible();
    {
      ...
      await expect(incidentInput).toBeVisible();
      {
