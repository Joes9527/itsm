...
describe('ServiceCatalogApi', () => {
  ...
  describe('getServices', () => {
    it('should get services list', async () => {
      mockGet.mockResolvedValue({ catalogs: [{ id: 1, name: 'Email', status: 'enabled', category: 'it_service', targetClass: 'service_request_item' }], total: 1 });
  ...
  describe('getService', () => {
    it('should get a single service', async () => {
      mockGet.mockResolvedValue({ id: 1, name: 'Email', status: 'enabled', targetClass: 'service_request_item' });
      const result = await ServiceCatalogApi.getService('1');
      expect(mockGet).toHaveBeenCalledWith('/api/v1/service-catalogs/1');
      expect(result.name).toBe('Email');
    });

    it('rejects a response with a missing targetClass', async () => {
      mockGet.mockResolvedValue({ id: 1, name: 'Email', status: 'enabled' });
      await expect(ServiceCatalogApi.getService('1')).rejects.toThrow('targetClass');
    });
  });

  describe('createService', () => {
    it('should create a service', async () => {
      mockPost.mockResolvedValue({ id: 2, name: 'VPN', status: 'enabled', targetClass: 'service_request_item' });
  ...
  describe('updateService', () => {
    it('should update a service', async () => {
      mockPut.mockResolvedValue({ id: 1, name: 'Updated', status: 'enabled', targetClass: 'service_request_item' });
    ...
    it('omits targetClass from the payload on update when not supplied', async () => {
      mockPut.mockResolvedValue({ id: 1, name: 'Renamed', status: 'enabled', targetClass: 'service_request_item' });
  ...
  describe('publishService', () => {
    it('should publish a service', async () => {
      mockPut.mockResolvedValue({ id: 1, status: 'enabled', targetClass: 'service_request_item' });
  ...
  describe('retireService', () => {
    it('should retire a service', async () => {
      mockPut.mockResolvedValue({ id: 1, status: 'disabled', targetClass: 'service_request_item' });
  ...
  describe('exportCatalog', () => {
    it('should export catalog as CSV', async () => {
      mockGet.mockResolvedValue({ services: [{ id: '1', name: 'Svc', category: 'IT', status: 'active', shortDescription: 'desc', targetClass: 'service_request_item', availability: { responseTime: '1h' }, createdAt: '2024-01-01', updatedAt: '2024-01-02' }], total: 1 });
  ...
  describe('cloneService', () => {
    it('should clone service', async () => {
      mockGet.mockResolvedValue({ id: '1', name: 'Original', category: 'IT', targetClass: 'service_request_item' });
      mockPost.mockResolvedValue({ id: '2', name: 'Clone', targetClass: 'service_request_item' });
