...
beforeEach(() => {
  ...
  get.mockImplementation(async (url: string) => {
    if (url.endsWith('/relation-context'))
      return { source: { ...item, workItemId: item.id }, mutation: { allowed: false } };
...
it.each(['generic', 'incident', 'problem', 'change_request'] as const)(
  ...
  async recordClass => {
    ...
    expect(get).toHaveBeenCalledWith('/api/v1/work-items/101/relations');
    expect(get).toHaveBeenCalledWith('/api/v1/work-items/101/relation-context');
