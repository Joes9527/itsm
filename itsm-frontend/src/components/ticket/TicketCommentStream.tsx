...
import {
  useDetailIdentity,
  useDetailResource,
} from '@/components/business/detail-tabs/useDetailResource';
 ...
 */
export const TicketCommentStream: React.FC<TicketCommentStreamProps> = props => {
  const identity = useDetailIdentity(props.ticketId);
  return <TicketCommentStreamContent key={identity} {...props} />;
};
const TicketCommentStreamContent: React.FC<TicketCommentStreamProps> = ({
  ticketId,
  onCountChange,
  currentUserId,
  ticketAssigneeId,
  ...
  if (!resource.ready) {
    return <DetailReadState {...resource} />;
  }

  return (
    <div className='space-y-4 pt-2'>
      <DetailReadState {...resource} />
      {/* 消息时间线 */}
      {comments.length > 0 ? (
