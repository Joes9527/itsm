...
it('opens a navigation group without leaving the page, then navigates via a child', async () => {
  ...
  render(<MenuItems selectedKeys={[]} onMenuClick={path => destinations.push(path)} items={[{
    key: '/workflow', path: '/workflow', label: '工作流', icon: null, children: [
      { key: '/admin/workflows', path: '/admin/workflows', label: '工作流管理', icon: null },
      { key: '/workflow/designer', path: '/workflow/designer', label: '流程设计器', icon: null },
      { key: '/workflow/instances', path: '/workflow/instances', label: '流程实例', icon: null },
    ],
  }]} />);
