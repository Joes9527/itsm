  ...
  switch (compType) {
    case 'Text': {
      ...
      const classes: Record<string, string> = {
        h1: 'text-[24px] font-semibold',
        h2: 'text-[20px] font-semibold',
        h3: 'text-[15px] font-medium',
        body: 'text-[13px]',
    ...
    default:
      return <div className="text-muted text-[12px]">Unknown: {compType}</div>;
  }
};
    ...
    return (
      <Card className="max-w-2xl mx-auto">
        <div className="text-center py-8">
          ...
          <h2 className="text-[20px] font-semibold mb-2">工单创建成功</h2>
          <p className="text-muted mb-4">您的工单已成功提交</p>
          <Button
            type="primary"
            onClick={() => {
              setSubmitted(false);
    ...
    <Card className="max-w-2xl mx-auto">
      ...
      {components.length > 0 ? (
        ...
        <div className="text-center text-muted py-8">
          ...
          <p className="text-[12px] mt-2">例如：帮我申请一台 ThinkPad 笔记本</p>
        </div>
      )}
      ...
      {process.env.NODE_ENV === 'development' && components.length > 0 && (
        <details className="mt-4 text-[12px] text-muted">
          ...
          <pre className="mt-2 p-2 bg-raised rounded overflow-auto max-h-40">
