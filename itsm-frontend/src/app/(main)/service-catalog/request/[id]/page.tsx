...
export default function ServiceCatalogRequestPage() {
  ...
  useEffect(() => {
    ...
    setFetchError(null);
    let cancelled = false;
    ServiceCatalogApi.getService(String(id))
      .then(data => {
        if (cancelled) return;
        if (
          !data.catalogVersion ||
          !data.formSchemaVersion ||
          !['generic', 'incident', 'problem', 'change_request', 'service_request_item'].includes(
            data.targetClass || ''
          )
        ) {
      ...
      .catch(() => {
        if (!cancelled) {
          setCatalog(null);
          setFetchError('服务信息加载失败，请重新读取并确认目录');
        }
      })
      .finally(() => {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      if (!catalog?.targetClass || !catalog.catalogVersion || !catalog.formSchemaVersion)
        throw new Error('请先读取并确认目录');
      ...
      if (serviceRequestTarget) {
        ...
        if (catalog.requiresInfraFields)
          Object.assign(payload, {
            costCenter: values.costCenter,
            dataClassification: values.dataClassification,
            needsPublicIp: !!values.needsPublicIp,
        ...
        if (payload.ciIds?.some(value => !Number.isInteger(value) || value <= 0))
          throw new Error('配置项 ID 必须为正整数');
        ...
        if (catalog.targetClass === 'incident')
          payload.incident = {
            ...
            impactAnalysis: values.incident?.impactAnalysis
              ? JSON.parse(values.incident.impactAnalysis)
              : undefined,
          };
        if (catalog.targetClass === 'change_request') {
          const change = values.change;
          if (change?.plannedStartDate && change?.plannedEndDate?.isBefore(change.plannedStartDate))
            throw new Error('结束时间必须晚于开始时间');
          payload.change = {
            ...change,
            plannedStartDate: change?.plannedStartDate?.toISOString(),
            plannedEndDate: change?.plannedEndDate?.toISOString(),
          };
        }
      }
      await creation.submit(payload, ServiceCatalogApi.createServiceRequest, receipt =>
        router.push(`/tickets/${receipt.workItemId}`)
      );
    } catch (e: any) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-[16px] md:p-[24px]">
      <Breadcrumb
      ...
      <Card>
        ...
        {(fetchError ||
          creation.attempts.some(
            attempt => attempt.state === 'rejected' || attempt.state === 'unknown'
          )) && (
          <Alert
            type="warning"
            className="mb-4"
            title="请重新读取目录并核对后再确认申请"
            description="重试不会自动替换原确认版本；重载将保留兼容答案，不兼容内容需您明确核对。"
            action={
              <Button
                loading={fetching}
                disabled={fetching || incompatibleAnswers.length > 0 || creation.submitting}
        ...
        {incompatibleAnswers.length > 0 && (
          <Alert
            type="warning"
            className="mb-4"
            title="目录变更需要重新核对已填内容"
            description={
              <>
                <p>
                  以下答案不再适用于新目录定义。确认后将舍弃这些答案，请在新表单重新填写；其他答案及原申请快照保留。
                </p>
                <ul>
                  {incompatibleAnswers.map(answer => (
                    <li key={JSON.stringify(answer.path)}>
                      {answer.label}：
        ...
        <Form
          ...
          layout="vertical"
          onFinish={onFinish}
          disabled={fetching || incompatibleAnswers.length > 0}
        >
          <CreationRequester />
