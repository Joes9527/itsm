...
export default function DashboardPage() {
  ...
  const controlMenuItems = [
    {
      ...
      label: (
        ...
        >
          <span className='text-[13px] font-medium'>自动刷新</span>
    ...
    },
    {
      ...
      label: (
        <div
          ...
          onClick={e => e.stopPropagation()}
        >
          <span className='text-[13px] font-medium'>刷新间隔</span>
          <Select
    ...
    { type: 'divider' as const },
    {
      ...
      label: (
        <div className='flex items-center justify-between min-w-[200px]'>
          <span className='text-[13px] font-medium'>连接状态</span>
          <Badge
    ...
    },
    {
      ...
      label: (
        <div className='flex items-center justify-between min-w-[200px]'>
          <span className='text-[13px] font-medium'>最后更新</span>
  ...
  if (error) {
    return (
      <Card className='text-center py-16 rounded-[8px] border-0 shadow-none'>
        <div className='text-red-500 mb-4 flex justify-center'>
          <LayoutDashboard size={64} />
        </div>
        <h3 className='text-[15px] font-semibold text-foreground mb-2'>仪表盘加载失败</h3>
  ...
  return (
    ...
    >
      ...
      <div className='flex items-center justify-between flex-wrap gap-3 mb-6 pb-4 border-b border-border'>
        <div>
          <h1 className='text-[24px] font-semibold text-foreground flex items-center gap-2'>
            AI-Native ITSM 运营仪表盘
            {isConnected && <Badge status='success' text='在线' />}
          </h1>
          <p className='text-[13px] text-muted mt-1'>实时监控系统运行状态和关键业务指标</p>
        </div>
        ...
        <>
          ...
          <div className='mb-6'>
            <div className='mb-4'>
              ...
              <p className='text-[13px] text-muted'>常用功能快捷入口，提升工作效率</p>
            </div>
            {(!data?.quickActions || data.quickActions.length === 0) && !loading ? (
              <Row gutter={[16, 16]}>
                <Col xs={24} sm={12} lg={6}>
                  <Card className='rounded-[8px] border border-border shadow-none transition-all h-full'>
                    <div className='flex flex-col items-center text-center py-4'>
                      <div className='w-12 h-12 bg-raised rounded-[8px] flex items-center justify-center text-blue-600 mb-3'>
                        <Ticket className='w-6 h-6' />
                      </div>
                      <h3 className='text-[15px] font-semibold text-foreground mb-1'>创建第一个工单</h3>
                      <p className='text-[13px] text-muted mb-4'>提交 IT 服务请求或报告问题</p>
                ...
                <Col xs={24} sm={12} lg={6}>
                  <Card className='rounded-[8px] border border-border shadow-none transition-all h-full'>
                    <div className='flex flex-col items-center text-center py-4'>
                      <div className='w-12 h-12 bg-raised rounded-[8px] flex items-center justify-center text-green-600 mb-3'>
                        <Compass className='w-6 h-6' />
                      </div>
                      <h3 className='text-[15px] font-semibold text-foreground mb-1'>浏览服务目录</h3>
                      <p className='text-[13px] text-muted mb-4'>查看可用的 IT 服务目录</p>
                ...
                <Col xs={24} sm={12} lg={6}>
                  <Card className='rounded-[8px] border border-border shadow-none transition-all h-full'>
                    <div className='flex flex-col items-center text-center py-4'>
                      <div className='w-12 h-12 bg-raised rounded-[8px] flex items-center justify-center text-purple-600 mb-3'>
                        <Database className='w-6 h-6' />
                      </div>
                      <h3 className='text-[15px] font-semibold text-foreground mb-1'>配置 CMDB</h3>
                      <p className='text-[13px] text-muted mb-4'>管理配置项和资产关系拓扑</p>
                ...
                <Col xs={24} sm={12} lg={6}>
                  <Card className='rounded-[8px] border border-border shadow-none transition-all h-full'>
                    <div className='flex flex-col items-center text-center py-4'>
                      <div className='w-12 h-12 bg-raised rounded-[8px] flex items-center justify-center text-amber-600 mb-3'>
                        <BookOpen className='w-6 h-6' />
                      </div>
                      <h3 className='text-[15px] font-semibold text-foreground mb-1'>查阅知识库</h3>
                      <p className='text-[13px] text-muted mb-4'>搜索解决方案和 IT 知识文档</p>
          ...
          {/* 图表分析区域 */}
          <div className='mb-6'>
            <Card className='rounded-[8px] border-0 shadow-none' styles={{ body: { padding: '24px' } }}>
              <div className='mb-5'>
                <h2 className='text-[15px] font-semibold text-foreground mb-1 flex items-center gap-2'>
                  <LineChart className='text-blue-500 w-5 h-5' />
                  数据分析与趋势
                </h2>
                <p className='text-[13px] text-muted'>系统性能和业务趋势的可视化分析</p>
              </div>
