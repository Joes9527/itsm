...
export function useAssignedTickets() {
  ...
  const selectedId =
    items?.some(item => item.id === navigation.selectedId) && navigation.identity === identity
      ? navigation.selectedId
      : items?.[0]?.id;
  // Persist the effective selection so a moved record does not steal focus when it later returns.
  useEffect(() => {
    setNavigation(previous =>
      previous.identity === identity && previous.page === page && previous.selectedId === selectedId
        ? previous
