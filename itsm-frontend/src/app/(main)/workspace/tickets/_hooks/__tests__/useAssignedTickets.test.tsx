...
const response = (ids: number[], total = ids.length): TicketListResponse => ({
  tickets: ids.map(id => ({ id, title: `Ticket ${id}`, ticketNumber: `TKT-${id}` })) as TicketListResponse['tickets'],
  total, page: 1, size: 20, pageSize: 20,
});
function session(id = 7, tenant = 1, permissions = ['ticket:read']) {
  useAuthStore.setState({user: {id, tenantId: tenant, actorTenantId: tenant, permissions} as SessionUser,
    currentTenant: {id: tenant, status: 'active'} as Tenant, isAuthenticated: true});
}
beforeEach(() => { jest.clearAllMocks(); session(); getTickets.mockResolvedValue(response([101, 102], 51)); });
...
it('clears old page and fences a late page response', async () => {
  ...
  getTickets.mockImplementationOnce(() => new Promise(resolve => {finish=resolve;}));
