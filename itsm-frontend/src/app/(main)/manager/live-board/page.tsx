...
export default function ManagerLiveBoardPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      {!clusteringDismissed && (
        <div className="p-4 rounded-[8px] bg-red-50/80 dark:bg-red-950/30 border border-red-200 dark:border-red-900/60 shadow-none flex flex-col md:flex-row md:items-center justify-between gap-4 animate-bounce-short">
          <div className="flex items-start gap-3">
            <div className="p-2 rounded-[8px] bg-red-100 dark:bg-red-900/60 text-red-600 dark:text-red-300 mt-0.5">
              <Flame size={20} />
            </div>
            <div>
              <div className="flex items-center gap-2">
              ...
              <p className="text-[12px] text-red-700 dark:text-red-300/80 mt-1 m-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-[8px] bg-surface border border-border shadow-none">
          <div className="text-[12px] font-semibold text-muted">今日进单量 / 解决量</div>
          <div className="text-[26px] font-semibold text-foreground mt-1 flex items-baseline gap-2">
            <span>42</span>
            <span className="text-[13px] font-normal text-muted">/ 38 已解决</span>
          </div>
        ...
        <div className="p-5 rounded-[8px] bg-surface border border-border shadow-none">
          <div className="text-[12px] font-semibold text-muted">当前 SLA 达成率</div>
          <div className="text-[26px] font-semibold text-emerald-600 mt-1">98.2%</div>
          <div className="text-[12px] text-muted mt-2">月度目标: ≥95.0%</div>
        </div>

        <div className="p-5 rounded-[8px] bg-surface border border-border shadow-none">
          <div className="text-[12px] font-semibold text-muted">平均首次响应时长 (FCR)</div>
          <div className="text-[26px] font-semibold text-foreground mt-1">8.5 分钟</div>
          <div className="text-[12px] text-emerald-600 font-medium mt-2">较昨日提速 12%</div>
        </div>

        <div className="p-5 rounded-[8px] bg-surface border border-border shadow-none">
          <div className="text-[12px] font-semibold text-muted">待分配工单池 (L1)</div>
          <div className="text-[26px] font-semibold text-amber-500 mt-1">12 张</div>
          <div className="text-[12px] text-amber-600 font-medium mt-2">建议触发负载均衡派发</div>
        </div>
      </div>
          ...
          {MOCK_TEAM_MEMBERS.map((mem) => (
            <div
              ...
              className={`p-4 rounded-[8px] border transition-all ${
                mem.isLightest
                  ? 'bg-emerald-50/40 dark:bg-emerald-950/20 border-emerald-300 dark:border-emerald-800 shadow-none'
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-[12px] font-semibold text-foreground">{mem.name}</div>
                    <div className="text-[11px] text-muted">{mem.status}</div>
                  </div>
                </div>
                {mem.isLightest && (
                  <span className="text-[10px] bg-emerald-100 dark:bg-emerald-900/60 text-emerald-700 dark:text-emerald-300 font-semibold px-1.5 py-0.5 rounded">
                    最闲待命
                  </span>
                )}
              </div>
